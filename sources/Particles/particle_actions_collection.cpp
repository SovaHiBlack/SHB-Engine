...
//-------------------------------------------------------------------------------------------------
void PAPI::PAAvoid::Execute(SParticleEffect* effect, float dt)
{
	...
	switch (position.type)
	{
		case PDPlane:
		{
			...
			{
				for (unsigned int i = 0; i < effect->p_count; i++)
				{
...
void PABounce::Execute(SParticleEffect* effect, float dt)
{
	switch (position.type)
	{
		case PDTriangle:
...
void PACopyVertexB::Execute(SParticleEffect* effect, float dt)
{
	unsigned int i;
...
// Dampen velocities
void PADamping::Execute(SParticleEffect* effect, float dt)
{
	// This is important if dt is != 1.
	CParticleVector one(1.0f, 1.0f, 1.0f);
...
void PAExplosion::Execute(SParticleEffect* effect, float dt)
{
...
void PAFollow::Execute(SParticleEffect* effect, float dt)
{
	float magdt = magnitude * dt;
	...
	{
		for (unsigned int i = 0; i < effect->p_count - 1; i++)
		{
			SParticle& m = effect->particles[i];
...
void PAGravitate::Execute(SParticleEffect* effect, float dt)
{
	float magdt = magnitude * dt;
	float max_radiusSqr = max_radius * max_radius;
	...
	if (max_radiusSqr < P_MAXFLOAT)
	{
		for (unsigned int i = 0; i < effect->p_count; i++)
		{
			...
			for (unsigned int j = i + 1; j < effect->p_count; j++)
			{
...
void PAGravity::Execute(SParticleEffect* effect, float dt)
{
...
void PAJet::Execute(SParticleEffect* effect, float dt)
{
	float magdt = magnitude * dt;
	float max_radiusSqr = max_radius * max_radius;
...
void PAScatter::Execute(SParticleEffect* effect, float dt)
{
	float magdt = magnitude * dt;
	float max_radiusSqr = max_radius * max_radius;
...
void PAKillOld::Execute(SParticleEffect* effect, float dt)
{
...
void PAMatchVelocity::Execute(SParticleEffect* effect, float dt)
{
	float magdt = magnitude * dt;
	float max_radiusSqr = max_radius * max_radius;
...
void PAMove::Execute(SParticleEffect* effect, float dt)
{
...
void PAOrbitLine::Execute(SParticleEffect* effect, float dt)
{
	float magdt = magnitude * dt;
	float max_radiusSqr = max_radius * max_radius;
...
void PAOrbitPoint::Execute(SParticleEffect* effect, float dt)
{
	float magdt = magnitude * dt;
	float max_radiusSqr = max_radius * max_radius;
...
void PARandomAccel::Execute(SParticleEffect* effect, float dt)
{
	for (unsigned int i = 0; i < effect->p_count; i++)
	{
		SParticle& m = effect->particles[i];
...
void PARandomDisplace::Execute(SParticleEffect* effect, float dt)
{
	for (unsigned int i = 0; i < effect->p_count; i++)
	{
		SParticle& m = effect->particles[i];
...
void PARandomVelocity::Execute(SParticleEffect* effect, float dt)
{
	for (unsigned int i = 0; i < effect->p_count; i++)
	{
		SParticle& m = effect->particles[i];
...
void PARestore::Execute(SParticleEffect* effect, float dt)
{
	if (time_left <= 0.0f)
	{
		for (unsigned int i = 0; i < effect->p_count; i++)
		{
			...
			m.pos = m.posB;
			m.vel = CParticleVector(0.0f, 0.0f, 0.0f);
		}
	}
	else
	{
...
void PASink::Execute(SParticleEffect* effect, float dt)
{
	// Must traverse list in reverse order so Remove will work
	for (int i = effect->p_count - 1; i >= 0; i--)
	{
...
void PASinkVelocity::Execute(SParticleEffect* effect, float dt)
{
	// Must traverse list in reverse order so Remove will work
	for (int i = effect->p_count - 1; i >= 0; i--)
	{
...
void PASource::Execute(SParticleEffect* effect, float dt)
{
	...
	if (m_Flags.is(unsigned int(flVertexB_tracks)))
	{
...
void PASpeedLimit::Execute(SParticleEffect* effect, float dt)
{
...
void PATargetColor::Execute(SParticleEffect* effect, float dt)
{
...
void PATargetSize::Execute(SParticleEffect* effect, float dt)
{
...
void PATargetRotate::Execute(SParticleEffect* effect, float dt)
{
	float scaleFac = scale * dt;
	...
	{
		SParticle& m = effect->particles[i];
		CParticleVector sign(m.rot.x >= 0.0f ? scaleFac : -scaleFac, m.rot.y >= 0.0f ? scaleFac : -scaleFac, m.rot.z >= 0.0f ? scaleFac : -scaleFac);
...
void PATargetVelocity::Execute(SParticleEffect* effect, float dt)
{
	float scaleFac = scale * dt;
...
void PAVortex::Execute(SParticleEffect* effect, float dt)
{
	float magdt = magnitude * dt;
	float max_radiusSqr = max_radius * max_radius;
...
// Turbulence
#include "noise.h"
void PATurbulence::Execute(SParticleEffect* effect, float dt)
{
	CParticleVector pV;
	CParticleVector vX;
	CParticleVector vY;
