...
inline void spatialParsFromDGeom(dGeomID d_space, Fvector3& center, Fvector3& AABB, float& radius)
{
	d_space->computeAABB( );
	dReal* dAABB = d_space->aabb;
	center.set((dAABB[0] + dAABB[1]) / 2.0f, (dAABB[2] + dAABB[3]) / 2.0f, (dAABB[4] + dAABB[5]) / 2.0f);
