...
struct CLoader
{
	...
	struct CHelper1
	{
		...
		template <>
		inline	static void load_data<true>(T& data, M& stream, const P& p)
		{
	...
	struct CHelper3
	{
		...
		struct add_helper
		{
			...
			template <>
			inline	static void add<true>(T1& data, T2& value)
			{
				data.insert(value);
			}
		};
	...
	inline	static void load_data(const char*& data, M& stream, const P& p)
	{
		NODEFAULT;
	}
...
namespace object_loader
{
	namespace detail
	{
		struct CEmptyPredicate
		{
			...
			{ }
			template <typename T1, typename T2>
			inline	bool operator()	(T1& data, const T2& value) const
			{
				return(true);
			}
			template <typename T1, typename T2>
			inline	bool operator()	(T1& data, const T2& value, bool) const
			{
				return(true);
			}
			inline	bool can_clear( ) const
			{
				return(true);
			}
			inline	bool can_add( ) const
			{
				return(true);
			}
		};
