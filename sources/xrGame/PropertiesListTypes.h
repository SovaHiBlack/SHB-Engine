...
public:
	...
	inline PropValue*		GetFrontValue	(){VERIFY(!values.empty()); return values.front(); };
	inline EPropType		Type			(){return type;}
	inline const char* Key				(){return key.c_str();}
...
{	return !!A.similar(B); }
inline void clamp(Fvector3& V, const Fvector3& mn, const Fvector3& mx)
{
	clamp(V.x,mn.x,mx.x);
	clamp(V.y,mn.y,mx.y);
	clamp(V.z,mn.z,mx.z);
}
inline xr_string draw_sprintf(xr_string& s, const Fvector3& V, int dec)
{
...
typedef NumericValue<unsigned short>	U16Value;
...
typedef TokenValue<unsigned short>	Token16Value;
...
typedef RTokenValue<unsigned short>RToken16Value;
