...
{
	static const	s32 base = 8;
	static const	s32 shift_to_variable = base / 2;
	static const	s32 mask_static = 0xf;
	flags8			flags;
...
class CPHIsland : public dxWorld
{
	...
	s32							m_nj;
	s32							m_nb;
	static const s32			JOINTS_LIMIT = 1500;
	static const s32			BODIES_LIMIT = 500;
...
public:
	...
	IC s32				MaxJoints(CPHIsland* island)
	{
		return MaxJoints( ) - island->nj;
	}

	IC s32				MaxBodies(CPHIsland* island)
	{
		return BODIES_LIMIT - nb - island->nb;
	}

	IC bool			CanMerge(CPHIsland* island, s32& MAX_JOINTS)
	{
