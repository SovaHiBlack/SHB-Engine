...
//const char* game_sv_GameState::get_name_id				(CClientID id)
//{
//	xrClientData*	C	= (xrClientData*)m_server->ID_to_client	(id);
//	if (0==C)			return 0;
//	else				return *C->name;
...
//const char* game_sv_GameState::get_player_name_id				(CClientID id)
//{
...
//unsigned short					game_sv_GameState::get_id_2_eid				(CClientID id)
//{
//	xrClientData*	C	= (xrClientData*)m_server->ID_to_client	(id);
...
//game_PlayerState*	game_sv_GameState::get_eid (unsigned short id) //if exist
//{
...
void* game_sv_GameState::get_client(unsigned short id) //if exist
{
	CSE_Abstract* entity = get_entity_from_eid(id);
...
CSE_Abstract* game_sv_GameState::get_entity_from_eid(unsigned short id)
{
...
//xr_vector<unsigned short>*		game_sv_GameState::get_children				(CClientID id)
//{
//	xrClientData*	C	= (xrClientData*)m_server->ID_to_client	(id);
//	if (0==C)			return 0;
//	CSE_Abstract* E	= C->owner;
...
void game_sv_GameState::net_Export_State(CNetPacket& P, CClientID to)
{
	...
	P.w_u16(unsigned short(p_count));
	...
	for (u32 p_it = 0; p_it < get_players_count( ); ++p_it)
	{
		...
		A->setName(p_name);
		unsigned short tmp_flags = A->flags__;
...
void game_sv_GameState::net_Export_Update(CNetPacket& P, CClientID id_to, CClientID id)
{
	...
	if (A)
	{
		unsigned short bk_flags = A->flags__;
		if (id == id_to)
		{
			A->setFlag(GAME_PLAYER_FLAG_LOCAL);
		}
...
void game_sv_GameState::OnPlayerDisconnect(CClientID /**id_who/**/, char*, unsigned short)
{
	signal_Syncronize( );
}
...
void game_sv_GameState::u_EventGen(CNetPacket& P, unsigned short type, unsigned short dest)
{
	P.w_begin(M_EVENT);
...
void game_sv_GameState::OnHit(unsigned short id_hitter, unsigned short id_hitted, CNetPacket& P)
{
...
void game_sv_GameState::OnEvent(CNetPacket& tNetPacket, unsigned short type, u32 time, CClientID sender)
{
	switch (type)
	{
		...
		case GAME_EVENT_PLAYER_DISCONNECTED:
		{
			...
			unsigned short		GameID = tNetPacket.r_u16( );
		...
		{
			unsigned short		id_dest = tNetPacket.r_u16( );
			unsigned short     id_src = tNetPacket.r_u16( );
...
void game_sv_GameState::AddDelayedEvent(CNetPacket& tNetPacket, unsigned short type, u32 time, CClientID sender)
{
...
//u32 game_sv_GameState::getRPcount (unsigned short team_idx)
//{
...
//RPoint game_sv_GameState::getRP (unsigned short team_idx, u32 rp_idx)
//{
...
void game_sv_GameState::teleport_object(CNetPacket& packet, unsigned short id)
{ }

void game_sv_GameState::add_restriction(CNetPacket& packet, unsigned short id)
{ }

void game_sv_GameState::remove_restriction(CNetPacket& packet, unsigned short id)
{ }

void game_sv_GameState::remove_all_restrictions(CNetPacket& packet, unsigned short id)
{ }
