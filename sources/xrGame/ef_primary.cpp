...
float CPersonalWeaponTypeFunction::ffGetValue( )
{
	...
	VERIFY(result < m_fMaxResultValue + 1.0f);
	return result;
}
...
u32 CPersonalMaxHealth::dwfGetDiscreteValue(u32 dwDiscretizationValue)
{
	...
	else
	{
		if (fTemp <= 30)
		{
			return iFloor(1 * float(dwDiscretizationValue) / 10 + 0.5f);
		}

		if (fTemp <= 50)
		{
			return iFloor(2 * float(dwDiscretizationValue) / 10 + 0.5f);
		}

		if (fTemp <= 80)
		{
			return iFloor(3 * float(dwDiscretizationValue) / 10 + 0.5f);
		}

		if (fTemp <= 100)
		{
			return iFloor(4 * float(dwDiscretizationValue) / 10 + 0.5f);
		}

		if (fTemp <= 150)
		{
			return iFloor(5 * float(dwDiscretizationValue) / 10 + 0.5f);
		}

		if (fTemp <= 250)
		{
			return iFloor(6 * float(dwDiscretizationValue) / 10 + 0.5f);
		}

		if (fTemp <= 500)
		{
			return iFloor(7 * float(dwDiscretizationValue) / 10 + 0.5f);
		}

		if (fTemp <= 750)
		{
			return iFloor(8 * float(dwDiscretizationValue) / 10 + 0.5f);
		}

		return iFloor(9 * float(dwDiscretizationValue) / 10 + 0.5f);
	}
}
...
float CEquipmentType::ffGetValue( )
{
	...
	VERIFY(float(result) < m_fMaxResultValue + 1.0f);
	return float(result);
}
...
float CItemDeterioration::ffGetValue( )
{
	if (ef_storage( ).non_alife( ).member_item( ))
	{
		...
		if (weapon)
		{
			return (1.0f - weapon->GetCondition( ));
		}
...
#pragma todo("Dima to Dima : Append MainWeaponPreference with non-ALife branch")
	...
	else
	{
		CSE_ALifeHumanAbstract* l_tpALifeHumanAbstract = smart_cast<CSE_ALifeHumanAbstract*>(ef_storage( ).alife( ).member( ));
		R_ASSERT2(l_tpALifeHumanAbstract, "Non-human object in EquipmentPreference evaluation function");
		return l_tpALifeHumanAbstract->brain( ).m_cpMainWeaponPreferences[ef_storage( ).m_pfMainWeaponType->dwfGetDiscreteValue(iFloor(ef_storage( ).m_pfMainWeaponType->ffGetMaxResultValue( ) + 0.5f))];
	}
}
...
float CEnemyDistanceToGraphPoint::ffGetValue( )
{
	...
	if (l_tpALifeDynamicObject->m_fDistance < 5.0f)
	{
		return 0.0f;
	}

	if (l_tpALifeDynamicObject->m_fDistance < 10.0f)
	{
		return 1.0f;
	}

	if (l_tpALifeDynamicObject->m_fDistance < 15.0f)
	{
		return 2.0f;
	}

	if (l_tpALifeDynamicObject->m_fDistance < 20.0f)
	{
		return 3.0f;
	}
