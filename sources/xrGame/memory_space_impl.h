...
template <typename T>
inline	bool CMemoryObject<T>::operator==		(unsigned short id) const
{
...
template <typename T>
inline	unsigned short object_id(const T *object)
{
	return					(object ? unsigned short(object->ID()) : unsigned short(0xffff));
}
