...
void CAISpace::load(pcstr level_name)
{
	unload(true);
...
void CAISpace::unload(bool reload)
{
...
#ifdef DEBUG
void CAISpace::validate(const u32 level_id) const
{
	...
	for (u32 i = 0, n = game_graph( ).header( ).vertex_count( ); i < n; ++i)
	{
		...
		CGameGraph::const_spawn_iterator	I;
		CGameGraph::const_spawn_iterator	E;
...
void CAISpace::patrol_path_storage_raw(IReader& stream)
{
	xr_delete(m_patrol_path_storage);
	m_patrol_path_storage = xr_new<CPatrolPathStorage>( );
...
void CAISpace::patrol_path_storage(IReader& stream)
{
	xr_delete(m_patrol_path_storage);
	m_patrol_path_storage = xr_new<CPatrolPathStorage>( );
...
void CAISpace::set_alife(CALifeSimulator* alife_simulator)
{
