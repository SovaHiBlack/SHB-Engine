//	Module 		: ALifeMonsterDetailPathManager.cpp
...
void CALifeMonsterDetailPathManager::follow_path(const ALife::_TIME_ID& time_delta)
{
...
#ifdef DEBUG
		object( ).m_fDistanceFromPoint = 0.0f;
		object( ).m_fDistanceToPoint = 0.0f;
	...
	float last_time_delta = float(time_delta) / 1000.0f;
	for (; m_path.size( ) > 1;)
	{
		setup_current_speed( );
