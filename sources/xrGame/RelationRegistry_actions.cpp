...
{
	CharacterGoodwill	friend_attack_goodwill;
	CharacterGoodwill	neutral_attack_goodwill;
	CharacterGoodwill	enemy_attack_goodwill;

	CharacterGoodwill	friend_attack_reputation;
	CharacterGoodwill	neutral_attack_reputation;
	CharacterGoodwill	enemy_attack_reputation;
...
{
	static CharacterGoodwill friend_kill_goodwill = pSettings->r_s32(ACTIONS_POINTS_SECT, "friend_kill_goodwill");
	static CharacterGoodwill neutral_kill_goodwill = pSettings->r_s32(ACTIONS_POINTS_SECT, "neutral_kill_goodwill");
	static CharacterGoodwill enemy_kill_goodwill = pSettings->r_s32(ACTIONS_POINTS_SECT, "enemy_kill_goodwill");
	static CharacterGoodwill community_member_kill_goodwill = pSettings->r_s32(ACTIONS_POINTS_SECT, "community_member_kill_goodwill");

	static CharacterReputationValue friend_kill_reputation = pSettings->r_s32(ACTIONS_POINTS_SECT, "friend_kill_reputation");
	static CharacterReputationValue neutral_kill_reputation = pSettings->r_s32(ACTIONS_POINTS_SECT, "neutral_kill_reputation");
	static CharacterReputationValue enemy_kill_reputation = pSettings->r_s32(ACTIONS_POINTS_SECT, "enemy_kill_reputation");
	...
	static CharacterGoodwill friend_fight_help_goodwill = pSettings->r_s32(ACTIONS_POINTS_SECT, "friend_fight_help_goodwill");
	static CharacterGoodwill neutral_fight_help_goodwill = pSettings->r_s32(ACTIONS_POINTS_SECT, "neutral_fight_help_goodwill");
	static CharacterGoodwill enemy_fight_help_goodwill = pSettings->r_s32(ACTIONS_POINTS_SECT, "enemy_fight_help_goodwill");

	static CharacterReputationValue friend_fight_help_reputation = pSettings->r_s32(ACTIONS_POINTS_SECT, "friend_fight_help_reputation");
	static CharacterReputationValue neutral_fight_help_reputation = pSettings->r_s32(ACTIONS_POINTS_SECT, "neutral_fight_help_reputation");
	static CharacterReputationValue enemy_fight_help_reputation = pSettings->r_s32(ACTIONS_POINTS_SECT, "enemy_fight_help_reputation");
	...
	//вычисление изменения репутации и рейтинга пока ведется только для актера
	...
	switch (action)
	{
		case ATTACK:
		{
			if (actor)
			{
				...
				SFightData* fight_data_from = FindFight(from->ID( ), true);
				...
				SFightData* fight_data = FindFight(to->ID( ), true);
				if (fight_data)
				{
			...
			if (stalker)
			{
				...
				{	//изменить отношение ко всем членам атакованой группы (если такая есть)
					...
					ChangeCommunityGoodwill(stalker->Community( ), from->ID( ), (CharacterGoodwill) (CCharacterCommunity::sympathy(stalker->Community( )) * (float) delta_goodwill));
				}
		...
		case KILL:
		{
			if (stalker)
			{
				...
				CharacterGoodwill community_goodwill =
					(CharacterGoodwill) (CCharacterCommunity::sympathy(stalker->Community( )) *
