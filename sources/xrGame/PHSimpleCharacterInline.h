...
void CPHSimpleCharacter::UpdateStaticDamage(dContact* c, SGameMtl* tri_material, bool bo1)
{
	...
	if (mag > m_collision_damage_info.m_contact_velocity)
	{
		...
		m_collision_damage_info.m_obj_id = unsigned short(-1);
	}
}

void CPHSimpleCharacter::UpdateDynamicDamage(dContact* c, unsigned short obj_material_idx, dBodyID b, bool bo1)
{
	...
	dReal Kself = norm_vel * norm_vel * m_mass / 2.0f;
	dReal Kobj = norm_obj_vel * norm_obj_vel * m.mass / 2.0f;

	dReal Pcnorm = dDOT(Pc, norm);
	dReal KK = Pcnorm * Pcnorm / (m_mass + m.mass) / 2.0f;
	dReal accepted_energy = Kself * m_collision_damage_factor + Kobj * object_damage_factor - KK;

	if (accepted_energy > 0.0f)
	{
		SGameMtl* obj_material = GMLib.GetMaterialByIdx(obj_material_idx);
		c_vel = dSqrt(accepted_energy / m_mass * 2.0f) * obj_material->fBounceDamageFactor;
	}
	else
	{
		c_vel = 0.0f;
	...
	{
		CPHShellHolder* obj = bo1 ? retrieveRefObject(c->geom.g2) : retrieveRefObject(c->geom.g1);
		VERIFY(obj);
		if (!obj->getDestroy( ))
		{
...
inline		void	CPHSimpleCharacter::foot_material_update(unsigned short	contact_material_idx, unsigned short	foot_material_idx)
{
	if (*p_lastMaterialIDX != unsigned short(-1) && GMLib.GetMaterialByIdx(*p_lastMaterialIDX)->Flags.test(SGameMtl::flPassable) && !b_foot_mtl_check)
	{
		return;
	}
