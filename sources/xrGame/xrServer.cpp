...
u32 xrServer::OnDelayedMessage(CNetPacket& P, CClientID sender)			// Non-Zero means broadcasting with "flags" as returned
{
...
u32 xrServer::OnMessage(CNetPacket& P, CClientID sender)			// Non-Zero means broadcasting with "flags" as returned
{
	if (g_pGameLevel && Level( ).IsDemoSave( ))
	{
		Level( ).Demo_StoreServerData(P.B.data, P.B.count);
...
void xrServer::SendTo_LL(CClientID ID, pvoid data, u32 size, u32 dwFlags, u32 dwTimeout)
{
...
void		xrServer::OnChatMessage(CNetPacket* P, xrClientData* CL)
{
//	string256 ChatMsg;
...
void xrServer::AddDelayedPacket(CNetPacket& Packet, CClientID Sender)
{
	DelayedPackestCS.Enter( );
