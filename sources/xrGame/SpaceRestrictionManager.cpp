//	Module 		: SpaceRestrictionManager.cpp
...
const unsigned int time_to_delete = 300000;
...
inline CSpaceRestrictionManager::CRestrictionPtr CSpaceRestrictionManager::restriction(ALife::_OBJECT_ID id)
{
	CLIENT_RESTRICTIONS::iterator I = m_clients->find(id);
	VERIFY(m_clients->end( ) != I);
...
bool CSpaceRestrictionManager::accessible(ALife::_OBJECT_ID id, unsigned int level_vertex_id, float radius)
{
	CRestrictionPtr client_restriction = restriction(id);
	if (client_restriction)
	{
...
unsigned int CSpaceRestrictionManager::accessible_nearest(ALife::_OBJECT_ID id, const Fvector3& position, Fvector3& result)
{
	CRestrictionPtr client_restriction = restriction(id);
...
{
	char m_temp[4096];
	for (unsigned int i = 0, n = _GetItemCount(*restrictions); i < n; ++i)
	{
...
inline void CSpaceRestrictionManager::join_restrictions(CSharedString& restrictions, CSharedString update)
{
	...
	for (unsigned int i = 0, n = _GetItemCount(*update), count = xr_strlen(m_temp2); i < n; ++i)
	{
...
inline void CSpaceRestrictionManager::difference_restrictions(CSharedString& restrictions, CSharedString update)
{
	...
	char m_temp2[4096];
	strcpy(m_temp2, "");
	for (unsigned int i = 0, n = _GetItemCount(*restrictions), count = 0; i < n; ++i)
	{
