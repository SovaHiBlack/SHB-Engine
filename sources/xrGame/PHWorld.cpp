...
void CPHWorld::SetStep(dReal s)
{
	...
	{
		f32	frame_time = Device.fTimeDelta;
...
void CPHWorld::Create( )
{
	...
	fVector3 level_size,level_center;
...
//	u32 res = iFloor((f32(dTime) / 1000 / fixed_step)+0.5f);
	return res;
};
...
void CPHWorld::FrameStep(dReal step)
{
	...
	u32 it_number;
	f32 frame_time = m_frame_time;
	frame_time += step;
	//m_frame_sum+=step;
#ifdef DEBUG
	if (ph_dbg_draw_mask.test(phDbgDrawObjectStatistics))
	{
		static f32 dbg_iterations = 0.f;
...
void CPHWorld::CutVelocity(f32 l_limit, f32 a_limit)
{
	PH_OBJECT_I			i_object;
	for (i_object = m_objects.begin( ); m_objects.end( ) != i_object;)
	{
