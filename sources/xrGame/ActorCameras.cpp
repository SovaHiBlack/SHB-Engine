...
void CActor::camUpdateLadder(f32 dt)
{
	...
	{
		cam_yaw += delta * _min(dt * 10.0f, 1.0f);
	}
	...
	if (es && es->State( ) == CElevatorState::clbClimbingDown)
	{
		...
		if (delta > 0.0f)
		{
			cam_pitch += delta * _min(dt * 10.0f, 1.0f);
		}
	}
}
...
void CActor::cam_Update(f32 dt, f32 fFOV)
{
	...
	if (this == Level( ).CurrentControlEntity( ))
	{
		if (!fis_zero(r_torso_tgt_roll))
		{
			...
			calc_point(tgt_pt, radius, 0.0f, alpha);
			src_pt.set(0.0f, tgt_pt.y, 0.0f);
			// init valid angle
	...
	if (!fis_zero(r_torso.roll))
	{
		...
		calc_point(point, radius, 0.0f, valid_angle);
