...
void CLevel::IR_OnKeyboardPress(int dik)
{
	...
	switch (_curr)
	{
		...
		case kQUIT:
		{
			...
			{
				if (HUD( ).GetUI( )->MainInputReceiver( )->IR_OnKeyboardPress(dik))
				{	//special case for main_menu
					return;
				}
	...
	if (b_ui_exist && pHUD->GetUI( )->IR_OnKeyboardPress(dik))
	{
		return;
	}
	...
	if (game && Game( ).IR_OnKeyboardPress(dik))
	{
		return;
	}
...
void CLevel::IR_OnKeyboardRelease(int dik)
{
	...
	if (b_ui_exist && pHUD->GetUI( )->IR_OnKeyboardRelease(dik))
	{
		return;
	}
	...
	if (game && Game( ).OnKeyboardRelease(get_binded_action(dik)))
	{
		return;
	}
...
void CLevel::IR_OnKeyboardHold(int dik)
{
	...
	if (b_ui_exist && pHUD->GetUI( )->IR_OnKeyboardHold(dik))
	{
		return;
	}
