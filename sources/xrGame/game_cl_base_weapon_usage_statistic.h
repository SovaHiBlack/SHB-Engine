#pragma once

#include "level_bullet_manager.h"
#include "game_base_kill_type.h"

struct SBulletData
{
	CSharedString		FirerName;
	CSharedString		WeaponName;

	unsigned short				HitRefCount;
	unsigned short				HitResponds;

	bool			Removed;

	SBullet			Bullet;
	bool			operator	==		(u32 BulletID)
	{
		return BulletID == Bullet.m_dwID;
	}
	bool			operator	!=		(u32 BulletID)
	{
		return BulletID != Bullet.m_dwID;
	}

	SBulletData(CSharedString FName, CSharedString WName, SBullet* pBullet);
};

using BulletsDataVec = xr_vector<SBulletData>;
using BulletsDataVec_it = BulletsDataVec::iterator;

struct SHitData
{
	Fvector3 Pos0;
	Fvector3 Pos1;

	signed short		BoneID;
	CSharedString	BoneName;
	unsigned short		TargetID;
	CSharedString	TargetName;
	u32		BulletID;
	bool	Deadly;

	bool	Completed;

	void			net_save(CNetPacket* P);
	void			net_load(CNetPacket* P);

	bool			operator	==		(u32 ID)
	{
		return ID == BulletID;
	}
	bool			operator	!=		(u32 ID)
	{
		return ID != BulletID;
	}
};

using HitsDataVec = xr_vector<SHitData>;
using HitsDataVec_it = HitsDataVec::iterator;

#define MAX_BASKET		34
struct SWeaponStatistic
{
	CSharedString		WName;
	CSharedString		InvName;
	u32				NumBought;
	//---------------------------
	u32				m_dwRoundsFired, m_dwRoundsFired_d;
	u32				m_dwBulletsFired, m_dwBulletsFired_d;
	u32				m_dwHitsScored, m_dwHitsScored_d;
	u32				m_dwKillsScored, m_dwKillsScored_d;
	//---------------------------
	u32				m_Basket[3][MAX_BASKET];

	u32				m_dwNumCompleted;
	HitsDataVec		m_Hits;

	SWeaponStatistic(const char* Name);
	~SWeaponStatistic( );

	void			net_save(CNetPacket* P);
	void			net_load(CNetPacket* P);

	bool			FindHit(u32 BulletID, HitsDataVec_it& Hit_it);
	bool			operator	==		(const char* name)
	{
		int res = xr_strcmp(WName.c_str( ), name);
		return res == 0;
	}
};

using WeaponsStatisticVec = xr_vector<SWeaponStatistic>;
using WeaponsStatisticVec_it = WeaponsStatisticVec::iterator;

struct SPlayerStatistic
{
	CSharedString		PName;

	u32				m_dwTotalShots;
	u32				m_dwTotalShots_d;
	//-----------------------------------------------
	u32				m_dwTotalAliveTime[3];
	int				m_dwTotalMoneyRound[3];
	u32				m_dwNumRespawned[3];
	unsigned char				m_dwArtefacts[3];

	u32				m_dwSpecialKills[3];//headshot, backstab, knifekill

	unsigned char				m_dwCurrentTeam;

	WeaponsStatisticVec	aWeaponStats;
	//-----------------------------------------------
	u32				m_dwCurMoneyRoundDelta;

	SPlayerStatistic(const char* Name);
	~SPlayerStatistic( );

	WeaponsStatisticVec_it	FindPlayersWeapon(const char* WeaponName);

	void			net_save(CNetPacket* P);
	void			net_load(CNetPacket* P);

	bool			operator	==		(const char* name)
	{
		int res = xr_strcmp(PName.c_str( ), name);
		return res == 0;
	}
};

using PlayersStatisticVec = xr_vector<SPlayerStatistic>;
using PlayersStatisticVec_it = PlayersStatisticVec::iterator;

struct SBulletCheckRequest
{
	u32 BulletID;
	signed short BoneID;
	bool Result;
	bool Processed;
	SBulletCheckRequest( ) : BulletID(0), Result(false)
	{ }
	SBulletCheckRequest(u32 ID, signed short BID) : BulletID(ID), BoneID(BID), Result(false), Processed(false)
	{ }
};

using BulletsCheckRequestVec = xr_vector<SBulletCheckRequest>;

struct SBulletCheckArray
{
	u32	SenderID;

	BulletsCheckRequestVec	Requests;
	unsigned char		NumTrue;
	unsigned char		NumFalse;

	bool			operator	==		(u32 ID)
	{
		return	ID == SenderID;
	}
	bool			operator	!=		(u32 ID)
	{
		return	ID != SenderID;
	}
	SBulletCheckArray(u32 ID) : SenderID(ID)
	{
		Requests.clear( );
		NumTrue = 0;
		NumFalse = 0;
	}
	~SBulletCheckArray( )
	{
		Requests.clear_and_free( );
	}
};

using BulletsCheckArrayVec = xr_vector<SBulletCheckArray>;
using BulletsCheckArrayVec_it = BulletsCheckArrayVec::iterator;

struct SWeaponUsageStatistic
{
	bool			m_bCollectStatistic;
	bool			CollectData( )
	{
		return m_bCollectStatistic;
	}
	void			SetCollectData(bool Collect);
	//-----------------------------------------------
	BulletsDataVec		ActiveBullets;
	//-----------------------------------------------
	PlayersStatisticVec	aPlayersStatistic;
	//-----------------------------------------------
	u32				m_dwTotalPlayersAliveTime[3];
	int				m_dwTotalPlayersMoneyRound[3];
	u32				m_dwTotalNumRespawns[3];
	//-----------------------------------------------
	u32				m_dwLastUpdateTime;
	u32				m_dwUpdateTimeDelta;
	//-----------------------------------------------
	SWeaponUsageStatistic( );
	~SWeaponUsageStatistic( );

	void				Clear( );

	PlayersStatisticVec_it				FindPlayer(const char* PlayerName);
	bool								GetPlayer(const char* PlayerName, PlayersStatisticVec_it& pPlayerI);
	void								ChangePlayerName(const char* from, const char* to);

	bool								FindBullet(u32 BulletID, BulletsDataVec_it& Bullet_it);
	void								RemoveBullet(BulletsDataVec_it& Bullet_it);
	//-----------------------------------------------
	void				OnWeaponBought(game_PlayerState* ps, const char* WeaponName);
	void				OnBullet_Fire(SBullet* pBullet, const CCartridge& cartridge);
	void				OnBullet_Hit(SBullet* pBullet, unsigned short TargetID, signed short element, Fvector3 HitLocation);
	void				OnBullet_Remove(SBullet* pBullet);
	//-----------------------------------------------

	u32						m_dwLastRequestSenderID;

	BulletsCheckArrayVec					m_Requests;

	void				OnBullet_Check_Request(SHit* pHDS);
	void				OnBullet_Check_Result(bool Result);
	//-----------------------------------------------
	void				Send_Check_Respond( );
	void				On_Check_Respond(CNetPacket* P);

	void				OnPlayerKilled(game_PlayerState* ps);
	void				OnPlayerSpawned(game_PlayerState* ps);
	void				OnPlayerAddMoney(game_PlayerState* ps, int MoneyAmount);
	void				OnPlayerBringArtefact(game_PlayerState* ps);
	void				OnPlayerKillPlayer(game_PlayerState* ps, KILL_TYPE KillType, SPECIAL_KILL_TYPE SpecialKillType);
	void				OnExplosionKill(game_PlayerState* ps, const SHit& hit);
	//-----------------------------------------------
	void				Update( );
	void				OnUpdateRequest(CNetPacket* P);
	void				OnUpdateRespond(CNetPacket* P);
	//-----------------------------------------------
	string_path			mFileName;
};

struct SBulletCheckRespondTrue
{
	u32 BulletID;
	signed short BoneID;
};
