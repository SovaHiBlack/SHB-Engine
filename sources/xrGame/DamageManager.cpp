...
void C_DamageManager::HitScale(const int element, float& hit_scale, float& wound_scale, bool aim_bullet)
{
	...
	if (aim_bullet)
	{
		scale = V->LL_GetBoneInstance(unsigned short(element)).get_param(3);
	}
	else
	{
		scale = V->LL_GetBoneInstance(unsigned short(element)).get_param(0);
	}

	hit_scale = scale;

	// get wound scale
	scale = V->LL_GetBoneInstance(unsigned short(element)).get_param(2);
	wound_scale = scale;
}
...
void C_DamageManager::load_section(const char* section, CConfigurationFile* ini)
{
	...
	for (CConfigurationFile::SectCIt i = damages.Data.begin( ); damages.Data.end( ) != i; ++i)
	{
		if (xr_strcmp(*(*i).first, "default"))
		{
			...
			CBoneInstance& bone_instance = kinematics->LL_GetBoneInstance(unsigned short(bone));
...
void C_DamageManager::init_bones(const char* section, CConfigurationFile* ini)
{
	CKinematics* kinematics = smart_cast<CKinematics*>(m_object->Visual( ));
	VERIFY(kinematics);
	for (unsigned short i = 0; i < kinematics->LL_BoneCount( ); i++)
	{
