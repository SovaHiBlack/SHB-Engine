...
bool CCar::DoorHit(float P, signed short element,ALife::EHitType hit_type)
{
	...
	{
		xr_map<unsigned short,SDoor>::iterator i=m_doors.begin(),e=m_doors.end();
		for(;e!=i;++i)i->second.Open();
	}
	xr_map   <unsigned short,SDoor>::iterator i=m_doors.find(element);
	if(i!=m_doors.end())
	{
		i->second.Hit(P);
		return true;
...
//	CBoneData& bone_data= pKinematics->LL_GetData(unsigned short(bone_id));
	CBoneInstance& bone_instance=pKinematics->LL_GetBoneInstance(unsigned short(bone_id));
...
void CCar::SDoor::ApplyDamage(unsigned short level)
{
