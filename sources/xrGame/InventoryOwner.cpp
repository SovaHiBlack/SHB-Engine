...
void CInventoryOwner::save(CNetPacket& output_packet)
{
	...
	{
		output_packet.w_u8((unsigned char) (-1));
	}
	else
	{
		output_packet.w_u8((unsigned char) inventory( ).GetActiveSlot( ));
	}
...
{
	unsigned char active_slot = input_packet.r_u8( );
	if (active_slot == unsigned char(-1))
	{
		inventory( ).SetActiveSlot(NO_ACTIVE_SLOT);
	}
	else
...
void CInventoryOwner::spawn_supplies( )
{
	...
	if (!ai( ).get_alife( ))
	{
		...
		R_ASSERT(pda);
		pda->m_original_owner = (unsigned short) game_object->ID( );
		CNetPacket P;
...
unsigned short CInventoryOwner::object_id( ) const
{
