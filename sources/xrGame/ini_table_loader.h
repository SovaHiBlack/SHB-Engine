...
#define TEMPLATE_SPECIALIZATION		template<typename T_ITEM, typename T_INI_LOADER, unsigned short TABLE_INDEX >
#define TEMPLATE_SPECIALIZATION_D	template<typename T_ITEM, typename T_INI_LOADER, unsigned short TABLE_INDEX = 0>
...
//	T_INI_LOADER::InitIdToIndex ();
	...
	char buffer[64];
	CConfigurationFile::Sect& table_ini = pSettings->r_section(table_sect);

	R_ASSERT3(table_ini.Data.size( ) == table_size, "wrong size for table in section", table_sect);

	for (CConfigurationFile::SectCIt i = table_ini.Data.begin( ); table_ini.Data.end( ) != i; ++i)
	{
