...
struct CMergeInOutPredicate
{
	...
	inline bool	operator()			(unsigned int level_vertex_id) const
	{
		if (!m_out || !m_in)
		{
			return false;
...
bool CSpaceRestriction::accessible(const Fsphere& sphere)
{
	...
	return (ai( ).level_graph( ).valid_vertex_position(sphere.P) &&
			(m_out_space_restriction ? (m_out_space_restriction->inside(sphere) && !m_out_space_restriction->on_border(sphere.P) && !m_out_space_restriction->out_of_border(sphere.P) ) : true) &&
...
{
	return (affect(bridge, start_vertex_id, 0.0f) || affect(bridge, dest_vertex_id, 0.0f));
}
