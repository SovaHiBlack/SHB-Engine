	...
	for ( ; I != E; ++I)
	{
		...
		if (::Render->ViewBase.testSphere_dirty(PC,sr)) {
			...
			if (bHL) {
				...
				if (S.z < 0)												continue;
				if (S.x < -1.f || S.x > 1.f || S.y < -1.f || S.y > 1.f)					continue;
...
void CLevelGraph::draw_restrictions	()
{
	...
	for ( ; I != E; ++I) {
		...
		unsigned char b = unsigned char(R.randI(255));
		unsigned char g = unsigned char(R.randI(255));
		unsigned char r = unsigned char(R.randI(255));
