...
const float wounded_enemy_reached_distance = 3.0f;
...
inline unsigned int population(const unsigned int& b)
{
	unsigned int a = b;
	a = (a & __c0) + ((a >> 1) & __c0);
	a = (a & __c1) + ((a >> 2) & __c1);
...
struct SEnemyFiller
{
	using ENEMIES = CAgentEnemyManager::ENEMIES;
	ENEMIES* m_enemies;
	squad_mask_type m_mask;

	inline			SEnemyFiller(ENEMIES* enemies, squad_mask_type mask)
	{
		m_enemies = enemies;
		m_mask = mask;
	}
...
struct SRemoveWoundedPredicate
{
...
void CAgentEnemyManager::fill_enemies( )
{
...

	{
		...
		{
			(*I)->probability(1.f);
			(*I)->object( ).memory( ).fill_enemies(SEnemyFiller(&m_enemies, object( ).member( ).mask(&(*I)->object( ))));
		}
	}
...
float CAgentEnemyManager::evaluate(const CEntityAlive* object0, const CEntityAlive* object1) const
{
	...
	return (ai( ).ef_storage( ).m_pfVictoryProbability->ffGetValue( ) / 100.0f);
}

void CAgentEnemyManager::exchange_enemies(CMemberOrder& member0, CMemberOrder& member1)
{
	unsigned int enemy0 = member0.selected_enemy( );
	unsigned int enemy1 = member1.selected_enemy( );
...
void CAgentEnemyManager::assign_enemies( )
{
	for (;;)
	{
		...
		(*i)->probability(best);
		(*I).m_probability *= 1.0f - best;

		// recovering sort order
		for (unsigned int i = 0, n = m_enemies.size( ) - 1; i < n; ++i)
		{
...
void CAgentEnemyManager::assign_wounded( )
{
	...
	unsigned int previous_wounded_count = m_wounded.size( );
	...
	unsigned int combat_member_count = population(object( ).member( ).combat_mask( ));
	VERIFY(combat_member_count == object( ).member( ).combat_members( ).size( ));

	unsigned int population_level = 0;
	while (population(assigned) < combat_member_count)
	{
		CMemberEnemy* enemy = nullptr;
		const CStalker* processor = nullptr;
		...
		if (!enemy)
		{
			...
			{
				using const_iterator = CAgentMemberManager::MEMBER_STORAGE::const_iterator;
...
struct SWoundedPredicate
{
	CObject* m_object;

	inline SWoundedPredicate(CObject* object)
	{
		VERIFY(object);
		m_object = object;
	}
...
class CFindWoundedPredicate
{
private:
	const CEntityAlive* m_object;

public:
	inline CFindWoundedPredicate(const CEntityAlive* object)
	{
		m_object = object;
		VERIFY(m_object);
	}
...
{
	VERIFY(value);
	WOUNDED_ENEMIES::iterator I = std::find_if(m_wounded.begin( ), m_wounded.end( ), CFindWoundedPredicate(object));
	if (I == m_wounded.end( ))
	{
		return;
	}
...
{
	WOUNDED_ENEMIES::const_iterator I = std::find_if(m_wounded.begin( ), m_wounded.end( ), CFindWoundedPredicate(object));
	if (I == m_wounded.end( ))
	{
		return false;
	}
