...
//		if (!(Game().local_player) || Game().local_player->testFlag(GAME_PLAYER_FLAG_VERY_VERY_DEAD)) return;
		if (CurrentControlEntity( ))
		{
			...
			if (!pObj->getDestroy( ) && pObj->net_Relevant( ))
			{
				...
				P.w_u16(unsigned short(pObj->ID( )));
...
#ifdef DEBUG
			u32 size = u32(Packet.w_tell( ) - position) - sizeof(unsigned short);
...
void			CLevel::OnBuildVersionChallenge( )
{
	CNetPacket P;
	P.w_begin(M_CL_AUTH);
	unsigned __int64 auth = FS.auth_get( );
	P.w_u64(auth);
	Send(P, net_flags(TRUE, TRUE, TRUE, TRUE));
}
...
void			CLevel::ClearAllObjects( )
{
	...
	while (ParentFound)
	{
		...
		for (u32 i = 0; i < CLObjNum; i++)
		{
			...
			GEN.w_u16(unsigned short(pObj->ID( )));
