...
CPHSimpleCharacter::CPHSimpleCharacter( )
{
	...
	m_ext_impuls_stop_step = unsigned __int64(-1);
	...
	b_jumping = false;
	b_death_pos = false;
	jump_up_velocity = 6.0f;
	m_air_control_factor = 0.0f;
	m_capture_joint = NULL;
	m_cap = NULL;
	m_cap_transform = NULL;
	dVectorSetZero(m_safe_velocity);
	m_collision_damage_factor = 1.0f;
...
{
	m_radius = _min(sizes[0], sizes[2]) / 2.0f;
	m_cyl_hight = sizes[1] - 2.0f * m_radius;
	if (m_cyl_hight < 0.0f)
	{
		m_cyl_hight = 0.01f;
	}
	...
	const dReal k = 1.2f;
	dReal doun = m_radius * _sqrt(1.0f - 1.0f / k / k) / 2.0f;
	...
	dGeomSetPosition(m_hat, 0.0f, m_cyl_hight, 0.0f);
	dGeomSetPosition(m_geom_shell, 0.0f, m_cyl_hight / 2.0f - doun, 0.0f);

	float test_radius = m_radius * 2.0f;
	float test_height = test_radius + m_radius / 2.0f;

	dGeomSphereSetRadius(m_cap, test_radius);
	dGeomSetPosition(m_cap, 0.0f, test_height, 0.0f);
}
...
void CPHSimpleCharacter::Create(dVector3 sizes)
{
	...
	/////////////////////////////////////////////////////////////////////////
	dMass m;
	dMassSetBox(&m, 1.0f, 1000000.0f, 1000000.0f, 1000000.0f);
...
const static unsigned __int64 impulse_time_constant = 30;
void CPHSimpleCharacter::ApplyImpulse(const Fvector3& dir, dReal P)
{
	...
	{
		m_ext_imulse.set(0.0f, -1.0f, 0.0f);
	}
...
void CPHSimpleCharacter::PhDataUpdate(dReal /**step/**/)
{
	...
	if (ph_world->m_steps_num > m_ext_impuls_stop_step)
	{
		...
		SetVelocity(Fvector3( ).set(0, 0, 0));
	}
	...
	m_last_move.mul(1.0f / fixed_step);
...
//#endif // def DEBUG
	...
	if (m_elevator_state.ClimbingState( ))
	{
		...
		if (b_stop_control)
		{
			dBodySetLinearVel(m_body, 0.0f, 0.0f, 0.0f);
		}
	}
	...
	if (b_lose_control && (b_on_ground && m_ground_contact_normal[1] > M_SQRT1_2 / 2.0f || fis_zero(linear_vel_smag) || m_elevator_state.ClimbingState( )))
	{
		b_lose_control = false;
	}
	...
	if (b_jumping)
	{
		...
		if (amag > 0.f)
		{
			...
			{
				dBodyAddForce(m_body, m_acceleration.x / amag * 1000.0f, 0, m_acceleration.z / amag * 1000.0f);
			}
		}
		...
		if (proj < 0.0f)
		{
			...
			dBodyAddForce(m_body, chVel[0] / vmag / amag * proj * 3000.0f, 0, chVel[2] / vmag / amag * proj * 3000.0f);
		}
	}

	BodyCutForce(m_body, 5.0f, 0.0f);

#ifdef DEBUG
	if (ph_dbg_draw_mask.test(phDbgCharacterControl))
	{
		DBG_DrawLine(cast_fv(dBodyGetPosition(m_body)), Fvector3( ).add(cast_fv(dBodyGetPosition(m_body)), Fvector3( ).mul(cast_fv(dBodyGetForce(m_body)), 1.0f / 19.6f)), D3DCOLOR_XRGB(255, 0, 128));
	}
#endif // def DEBUG
...
bool CPHSimpleCharacter::ValidateWalkOnMesh( )
{
	...
	Fvector3 obb_fb;
	obb_fb.set(m_radius * 0.5f, m_radius * 2.0f, m_radius * 0.7f);
	Fvector3 obb;
...
static const float pull_force = 25.0f;
void CPHSimpleCharacter::ApplyAcceleration( )
{
	...
	dVector3 accel = { m_acceleration.x, 0.0f, m_acceleration.z };
	if (m_elevator_state.Active( ))
	{
		if (m_elevator_state.NearState( ))
		{
			m_elevator_state.GetControlDir(*(Fvector3*) accel);
		}
		...
		if (m_elevator_state.ClimbingState( ))
		{
			...
			{
				dVectorMul(m_control_force, m_friction_factor * m.mass * pull_force * 2.0f);
				return;
			}
		}
	}
	...
	{
		dVectorMul(m_control_force, 4.0f);
		m_control_force[1] = dFabs(m_control_force[1]);
		m_control_force[0] = m_control_force[0] * accel[0] >= 0.0f ? m_control_force[0] : -m_control_force[0];
		m_control_force[2] = m_control_force[2] * accel[2] >= 0.0f ? m_control_force[2] : -m_control_force[2];
	}
...
void CPHSimpleCharacter::SetMas(dReal mass)
{
	...
	dMass m;
	dMassSetSphere(&m, 1.0f, 1000000.0f / 2.0f);
	dMassAdjust(&m, mass);
	dBodySetMass(m_body, &m);
}
...
void CPHSimpleCharacter::SafeAndLimitVelocity( )
{
	...
	if (dV_valid(linear_velocity))
	{
		...
		if (b_external_impulse)
		{
			...
			float ll_limit = m_ext_imulse.dotproduct(cast_fv(linear_velocity)) * 10.0f / fixed_step;
			if (sq_mag > EPS_L)
			{
				Fvector3 acc;
...
////////////////////////////////////////////////////////////////////////////////////////////
		...
		if (mag > l_limit)
		{
			if (!fis_zero(l_limit))
			{
				...
				{
					dBodySetLinearVel(m_body, linear_velocity[0] / f, linear_velocity[1], linear_velocity[2] / f);
				}
				else
				{
					CutVelocity(l_limit, 0.0f);
				}
...
void CPHSimpleCharacter::InitContact(dContact* c, bool& do_collide, unsigned short material_idx_1, unsigned short material_idx_2)
{
	...
	{
		c->surface.mu = 0.f;
		c->surface.soft_cfm = world_cfm * 2.0f;
	...
	float soft_param = dumping_rate + normal[1] * (1.0f - dumping_rate);
	if (is_control)
	{
	...
	{
		MulSprDmp(c->surface.soft_cfm, c->surface.soft_erp, spring_rate, soft_param);
		c->surface.mu *= (1.f + b_clamb_jump * 3.0f) * m_friction_factor;
	}
...
void CPHSimpleCharacter::FootProcess(dContact* c, bool& do_collide, bool bo)
{
	...
	{
		if (g == m_wheel && sign * normal[1] < 0.0f)
		{
			do_collide = false;
		}
	...
	if (dXZDot(m_acceleration, cast_fv(normal)) * sign > 0.0f)
	{
		return;
	}
...
static float	restrictor_depth = 0.0f;
