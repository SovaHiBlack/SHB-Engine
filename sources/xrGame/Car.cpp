...
void CCar::RestoreNetState(CSE_PHSkeleton* po)
{
	...
	////////////////////////////////////////////////////////////////////
	{
		...
		activation_shape.Activate(obj_size, 1, 1.0f, M_PI / 8.0f);
		Fvector3 dd;
...
*/
		if (HUD( ).GetUI( ))//
		{
			...
			HUD( ).GetUI( )->UIMainIngameWnd->CarPanel( ).SetCarHealth(GetfHealth( )/* / 100.0f */);
			HUD( ).GetUI( )->UIMainIngameWnd->CarPanel( ).SetSpeed(lin_vel.magnitude( ) / 1000.0f * 3600.0f / 100.0f);
			HUD( ).GetUI( )->UIMainIngameWnd->CarPanel( ).SetRPM(m_current_rpm / m_max_rpm / 2.0f);
		}
	}
...
void CCar::Hit(SHit* pHDS)
{
	...
	{
		C_DamageManager::HitScale(HDS.bone( ), hitScale, woundScale);
	}
...
//	VisualUpdate( );
	...
	ReleaseHandBreak( );
//	HUD( ).GetUI( )->UIMainIngameWnd->CarPanel( ).Show(true);
//	HUD( ).GetUI( )->UIMainIngameWnd->CarPanel( ).SetCarHealth(fEntityHealth / 100.0f);
...
//	CExplosive::SetInitiator(ID( ));
	...
	R_ASSERT2(m_power_neutral_factor > 0.1f && m_power_neutral_factor < 1.0f, "power_neutral_factor must be 0 - 1 !!");
	...
	for (int i = 1; true; ++i)
	{
		...
		gear_rat[1] *= (1.0f / 60.0f * 2.0f * M_PI);
		gear_rat[2] *= (1.0f / 60.0f * 2.0f * M_PI);
...
//	SWheel& ref_wheel=m_wheels_map.find(pKinematics->LL_BoneID(ini->r_string("car_definition","reference_wheel")))->second;
	...
	if (ini->line_exist("car_definition", "steer"))
	{
		...
		VERIFY2(fsimilar(DET(pKinematics->LL_GetTransform(m_bone_steer)), 1.0f, EPS_L), "BBADD MTX");
...
//	ref_wheel.Init( );
	...
	float l_time_to_explosion = READ_IF_EXISTS(ini, r_float, "car_definition", "time_to_explosion", 120.0f);
	...
	{
		xr_vector<SWheelDrive>::iterator i;
		xr_vector<SWheelDrive>::iterator e;
	...
	{
		xr_vector<SWheelBreak>::iterator i;
		xr_vector<SWheelBreak>::iterator e;
	...
	{
		xr_vector<SWheelSteer>::iterator i;
		xr_vector<SWheelSteer>::iterator e;
	...
	{
		xr_vector<SExhaust>::iterator i;
		xr_vector<SExhaust>::iterator e;
		i = m_exhausts.begin( );
		e = m_exhausts.end( );
		for (; i != e; ++i)
		{
...
{
	CDelayedActionFuse::Initialize(float(et) / 1000.0f, CDamagableItem::DamageLevelToHealth(2));
}
