...
const Fvector3 zero_vel = { 0.f,0.f,0.f };

CParticlesObject::CParticlesObject(const char* p_name, BOOL bAutoRemove, bool destroy_on_game_load) : inherited(destroy_on_game_load)
{
...
const CSharedString CParticlesObject::Name( )
{
	IParticleCustom* V = smart_cast<IParticleCustom*>(renderable.visual); VERIFY(V);
	return (V) ? V->Name( ) : "";
}
...
void CParticlesObject::shedule_Update(unsigned int _dt)
{
	...
	unsigned int dt = Device.dwTimeGlobal - dwLastTime;
	if (dt)
	{
		if (0)
		{
...
void CParticlesObject::PerformAllTheWork(unsigned int _dt)
{
	// Update
	unsigned int dt = Device.dwTimeGlobal - dwLastTime;
	if (dt)
