...
void CCameraLook::Update(fVector3& point, fVector3& /*noise_dangle*/)
{
	...
	f32 d = psCamSlideInert * prev_d + (1.0f - psCamSlideInert) * (R.range - covariance);
	prev_d = d;
...
void CCameraLook::Move(s32 cmd, f32 val, f32 factor)
{
	switch (cmd)
	{
		case kCAM_ZOOM_IN:
