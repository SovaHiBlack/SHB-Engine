...
void CUITaskItem::SendMessage(CUIWindow* pWnd, s16 msg, pvoid pData)
{
...
void CUITaskItem::OnItemClicked(CUIWindow*, pvoid)
{
...
void CUITaskRootItem::OnSwitchDescriptionClicked(CUIWindow*, pvoid)
{
...
void CUITaskSubItem::SetGameTask(CGameTask* gt, u16 obj_idx)
{
	...
	f32 h = _max(m_ActiveObjectiveStatic->GetWndPos( ).y + m_ActiveObjectiveStatic->GetHeight( ), m_descriptionStatic->GetWndPos( ).y + m_descriptionStatic->GetHeight( ));
	SetHeight(h);
...
void CUITaskSubItem::OnShowDescriptionClicked(CUIWindow*, pvoid)
{
	m_EventsWnd->ShowDescription(GameTask( ), ObjectiveIdx( ));
}
