...
void CUITalkDialogWnd::OnQuestionClicked(CUIWindow* w, pvoid)
{
	m_ClickedQuestionID = ((CUIQuestionItem*)w)->m_s_value;
	GetMessageTarget( )->SendMessage(this, TALK_DIALOG_QUESTION_CLICKED);
}

void CUITalkDialogWnd::OnTradeClicked(CUIWindow* w, pvoid)
{
...
void CUITalkDialogWnd::SendMessage(CUIWindow* pWnd, s16 msg, pvoid pData)
{
...
void CUITalkDialogWnd::AddQuestion(pcstr str, pcstr value)
{
...
void CUITalkDialogWnd::AddAnswer(pcstr SpeakerName, pcstr str, bool bActor)
{
...
void CUIQuestionItem::SendMessage(CUIWindow* pWnd, s16 msg, pvoid pData)
{
	CUIWndCallback::OnEvent(pWnd, msg, pData);
}

CUIQuestionItem::CUIQuestionItem(CUIXml* xml_doc, pcstr path)
{
...
void CUIQuestionItem::Init(pcstr val, pcstr text)
{
	m_s_value = val;
	m_text->SetText(text);
...
void CUIAnswerItem::Init(pcstr text, pcstr name)
{
	m_name->SetText(name);
	m_text->SetText(text);
	m_text->AdjustHeightToText( );
...
CUIAnswerItemIconed::CUIAnswerItemIconed(CUIXml* xml_doc, pcstr path) : CUIAnswerItem(xml_doc, path)
{
