...
void CUISequenceVideoItem::Load(CUIXml* xml, int idx)
{
	...
	const char* str2 = xml->Read("pause_sound", 0, "ignore");
...
void CUISequenceVideoItem::Update( )
{
	...
	if (m_texture)
	{
		...
		{	// sync start
			...
			{
				m_sound[0].play_at_pos(NULL, Fvector3( ).set(-0.5f, 0.f, 0.3f), sm_2D);
				m_sound[1].play_at_pos(NULL, Fvector3( ).set(+0.5f, 0.f, 0.3f), sm_2D);
