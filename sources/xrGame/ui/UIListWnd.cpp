...
void CUIListWnd::Init(float x, float y, float width, float height, float item_height)
{
	...
	m_ScrollBar->SetPageSize(signed short(0));
	m_ScrollBar->SetScrollPos(signed short(m_iFirstShownIndex));
...
void CUIListWnd::RemoveItem(int index)
{
	...
	{
		m_ScrollBar->SetRange(0, signed short(m_ItemList.size( ) - 1));
	}
	else
	{
		m_ScrollBar->SetRange(0, 0);
	...
	m_ScrollBar->SetPageSize(signed short((u32) m_iRowNum < m_ItemList.size( ) ? m_iRowNum : m_ItemList.size( )));
	m_ScrollBar->SetScrollPos(signed short(m_iFirstShownIndex));
	m_ScrollBar->Refresh( );
...
void CUIListWnd::SendMessage(CUIWindow* pWnd, signed short msg, void* pData)
{
	if (pWnd == m_ScrollBar)
	{
...
{
	m_ScrollBar->SetScrollPos((signed short) m_ScrollBar->GetMinRange( ));
	m_iFirstShownIndex = m_ScrollBar->GetScrollPos( );
	UpdateList( );
}
