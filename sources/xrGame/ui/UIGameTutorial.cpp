...
void CUISequenceItem::Load(CUIXml* xml, s32 idx)
{
	...
	s32 disabled_cnt = xml->GetNodesNum(xml->GetLocalRoot( ), "disabled_key");
	for (s32 i = 0; i < disabled_cnt; ++i)
	{
		pcstr str = xml->Read("disabled_key", i, NULL);
	...
	bool		functor_exists;
	s32			j;
	s32			f_num = xml->GetNodesNum(xml->GetLocalRoot( ), "function_on_start");
...
bool CUISequenceItem::AllowKey(s32 dik)
{
	xr_vector<s32>::iterator it = std::find(m_disabled_actions.begin( ), m_disabled_actions.end( ), get_binded_action(dik));
...
void CUISequencer::Start(pcstr tutor_name)
{
	...
	s32 items_count = uiXml.GetNodesNum(tutor_name, 0, "item");
	...
	for (s32 i = 0; i < items_count; ++i)
	{
		pcstr	_tp = uiXml.ReadAttrib("item", i, "type", "");
...
void CUISequencer::IR_OnMousePress(s32 btn)
{
...
void CUISequencer::IR_OnMouseRelease(s32 btn)
{
	if (!GrabInput( ) && m_pStoredInputReceiver)
	{
...
void CUISequencer::IR_OnMouseHold(s32 btn)
{
	if (!GrabInput( ) && m_pStoredInputReceiver)
	{
...
void CUISequencer::IR_OnMouseMove(s32 x, s32 y)
{
	if (!GrabInput( ) && m_pStoredInputReceiver)
	{
...
void CUISequencer::IR_OnMouseStop(s32 x, s32 y)
{
	if (!GrabInput( ) && m_pStoredInputReceiver)
	{
...
void CUISequencer::IR_OnKeyboardRelease(s32 dik)
{
	if (!GrabInput( ) && m_pStoredInputReceiver)
	{
...
void CUISequencer::IR_OnKeyboardHold(s32 dik)
{
	if (!GrabInput( ) && m_pStoredInputReceiver)
	{
...
void CUISequencer::IR_OnMouseWheel(s32 direction)
{
	if (!GrabInput( ) && m_pStoredInputReceiver)
	{
...
void CUISequencer::IR_OnKeyboardPress(s32 dik)
{
	if (m_items.size( ))
	{
