...
public:
	...
	static bool				InitFont				(CUIXml& xml_doc, const char* path, int index, unsigned int& color, CGameFont*& pFnt);
	...
	static unsigned int		GetColor				(CUIXml& xml_doc, const char* path, int index, unsigned int def_clr);
...
public:
	...
	static float			ApplyAlignX				(float coord, unsigned int align);
	static float			ApplyAlignY				(float coord, unsigned int align);
	static void				ApplyAlign				(float& x, float& y, unsigned int align);

	// Initialize and store predefined colors
	using ColorDefs									= xr_map<CSharedString, unsigned int>;
