...
struct S2DVert
{
	...
	S2DVert( )
	{ }
	S2DVert(f32 pX, f32 pY, f32 tU, f32 tV)
	{
		pt.set(pX, pY);
		uv.set(tU, tV);
	}
	void		set(f32 pt_x, f32 pt_y, f32 uv_x, f32 uv_y)
	{
		pt.set(pt_x, pt_y);
		uv.set(uv_x, uv_y);
	}
...
class ui_core : public CDeviceResetNotifier
{
	...
	IC f32		ClientToScreenScaledX(f32 left)
	{
		return (left * m_current_scale->x);
	}
	IC f32		ClientToScreenScaledY(f32 top)
	{
