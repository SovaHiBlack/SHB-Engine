...
void CUIFrameRect::Init(const char* texture, float x, float y, float w, float h)//, DWORD align)
{
	//SetPos(x, y);
	//SetSize(w, h);
	//SetAlign(align);
...
void CUIFrameRect::InitTexture(const char* texture)
{
	...
	if (FS.exist(fn, "$game_textures$", buf, ".ini"))
	{
		...
		CConfigurationFile* ini = CConfigurationFile::Create(fn, TRUE);
		const char* sh = ini->r_string("frame", "shader");
		...
		CConfigurationFile::Destroy(ini);
	}
	else
	{
...
void CUIFrameRect::Draw( )
{
	...
	{
		if (m_itm_mask.test(unsigned short(1 << k)))
		{
			frame[k].Render( );
		}
	}
