...
public:
	...
	typedef enum{NORMAL_PRESS, //кнопка нажимается при нажатии и отпускании на ней мыши
	...
	void				SetAccelerator			(s32 iAccel, s32 idx)	{VERIFY(idx==0||idx==1); m_uAccelerator[idx] = iAccel; }
	const s32			GetAccelerator			(s32 idx) const			{VERIFY(idx==0||idx==1); return m_uAccelerator[idx]; }
	IC bool				IsAccelerator			(s32 iAccel) const		{return (m_uAccelerator[0]==iAccel)||(m_uAccelerator[1]==iAccel) ;}
