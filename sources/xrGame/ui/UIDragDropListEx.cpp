...
void CUIDragDropListEx::SendMessage(CUIWindow* pWnd, s16 msg, pvoid pData)
{
...
void CUIDragDropListEx::OnItemStartDragging(CUIWindow* w, pvoid pData)
{
...
void CUIDragDropListEx::OnItemDrop(CUIWindow* w, pvoid pData)
{
	OnItemSelected(w, pData);
	CUICellItem* itm = smart_cast<CUICellItem*>(w);
...
void CUIDragDropListEx::OnItemDBClick(CUIWindow* w, pvoid pData)
{
	OnItemSelected(w, pData);
	CUICellItem* itm = smart_cast<CUICellItem*>(w);
...
void CUIDragDropListEx::OnItemSelected(CUIWindow* w, pvoid pData)
{
...
void CUIDragDropListEx::OnItemRButtonClick(CUIWindow* w, pvoid pData)
{
	OnItemSelected(w, pData);
	CUICellItem* itm = smart_cast<CUICellItem*>(w);
...
void CUIDragDropListEx::ReinitScroll( )
{
	...
	m_container->SetWndPos(0.0f, 0.0f);
}
...
void CUICellContainer::PlaceItemAtPos(CUICellItem* itm, iVector2& cell_pos)
{
	...
	for (s32 x = 0; x < cs.x; ++x)
	{
		for (s32 y = 0; y < cs.y; ++y)
		{
...
{
	for (s32 x = 0; x < m_cellsCapacity.x; ++x)
	{
		for (s32 y = 0; y < m_cellsCapacity.y; ++y)
		{
			iVector2 p;
			p.set(x, y);
...
{
	res.clear_not_free( );
	for (s32 x = rect.x1; x <= rect.x2; ++x)
	{
		for (s32 y = rect.y1; y <= rect.y2; ++y)
		{
...
#define tx 0.5f
	...
	{
		for (s32 y = 0; y <= tgt_cells.height( ); ++y)
		{
			fVector2 tp;
