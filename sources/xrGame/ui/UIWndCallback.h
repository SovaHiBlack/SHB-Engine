...
public:
	...
	virtual void		OnEvent				(CUIWindow* pWnd, signed short msg, void* pData = nullptr);
	void				Register			(CUIWindow* pChild);
	void				AddCallback			(const char* control_id, signed short event, const void_function& f);
	void				AddCallback			(const CSharedString& control_id, signed short event, const void_function& f);
};
