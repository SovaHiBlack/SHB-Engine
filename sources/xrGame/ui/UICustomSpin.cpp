...
void CUICustomSpin::Init(f32 x, f32 y, f32 width, f32 height)
{
	...
	m_pFrameLine->Init(0.0f, 0.0f, width, SPIN_HEIGHT);
	m_pFrameLine->InitTexture("ui_spiner");
	m_pBtnUp->Init(width - BTN_SIZE - 1.0f, 0.0f, BTN_SIZE, BTN_SIZE);
	m_pBtnUp->InitTexture("ui_spiner_button_t");
	m_pBtnDown->Init(width - BTN_SIZE - 1.0f, BTN_SIZE + 1.0f, BTN_SIZE, BTN_SIZE);
	m_pBtnDown->InitTexture("ui_spiner_button_b");

	m_pLines->Init(0.0f, 0.0f, width - BTN_SIZE - 10.0f, SPIN_HEIGHT);
}

void CUICustomSpin::SendMessage(CUIWindow* pWnd, s16 msg, pvoid pData)
{
