...
void CUICustomItem::Render(FVF::TL*& Pointer, const Fvector2& pos, u32 color, float x1, float y1, float x2, float y2)
{
	...
	S.resize(4);
	// LT
	S[0].set(LTp.x - 0.5f, LTp.y - 0.5f, LTt.x, LTt.y);
	// RT
	S[1].set(RBp.x - 0.5f, LTp.y - 0.5f, RBt.x, LTt.y);
	// RB
	S[2].set(RBp.x - 0.5f, RBp.y - 0.5f, RBt.x, RBt.y);
	// LB
	S[3].set(LTp.x - 0.5f, RBp.y - 0.5f, LTt.x, RBt.y);
...
void CUICustomItem::Render(FVF::TL*& Pointer, const Fvector2& pos_ns, u32 color, float angle)
{
	...
	{
		pivot.set(iVisRect.x2 / 2.0f, iVisRect.y2 / 2.0f);
	}
	else
	{
