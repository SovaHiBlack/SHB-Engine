...
#pragma todo("Dima to Dima : forward-back bone impulse direction has been determined incorrectly!")
		...
		float power_factor = perc / 100.f; clamp(power_factor, 0.0f, 1.0f);
...
void CActor::SetZoomRndSeed(int Seed)
{
	...
	{
		m_ZoomRndSeed = int(Level( ).timeServer_Async( ));
	}
}
...
void CActor::SetShotRndSeed(int Seed)
{
	...
	{
		m_ShotRndSeed = int(Level( ).timeServer_Async( ));
	}
}
