...
unsigned short					storyId2GameId(ALife::_STORY_ID);
...
unsigned short storyId2GameId(ALife::_STORY_ID id)
{
	if (ai( ).get_alife( ))
	{
		CSE_ALifeDynamicObject* so = ai( ).alife( ).story_objects( ).object(id, true);
		return (so) ? so->ID : unsigned short(-1);
	}
	else
	{
...
void CGameTask::Load(const TASK_ID& id)
{
	...
	for (int i = 0; i < tag_num; i++)
	{
		...
		objective.object_id = unsigned short(-1);
...
void SGameTaskObjective::SetObjectID_script(unsigned short id)
{
	object_id = id;
}
