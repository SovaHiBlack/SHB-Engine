...
s32			ph_tri_clear_disable_count = 10;
dWorldID	phWorld;
...
IC static s32 CollideIntoGroup(dGeomID o1, dGeomID o2, dJointGroupID jointGroup, CPHIsland* world, const s32& MAX_CONTACTS)
{
	const s32 RS = 800 + 10;
	const s32 N = RS;

	static dContact contacts[RS];
	s32	collided_contacts = 0;
...
{
	dMass m;
	dMassSetSphere(&m, 1.0f, ext_param);
...
f32 E_NLD(dBodyID b1, dBodyID b2, const dReal* norm)// norm - from 2 to 1
{
	...
	{
		return 0.0f;
	}//exit if the bodies are departing
