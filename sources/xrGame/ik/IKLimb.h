...
public:
							CIKLimb				();
				void		Create				(unsigned short id, CKinematics* K, const unsigned short bones[3], const Fvector3& toe_pos, bool collide_ );
				void		Destroy				( );
				void		Calculate			( SCalculateData& cd );
				void		Update				( CGameObject *O, const	CBlend *b, unsigned short interval );
inline				unsigned short			get_id				()	{ return m_id; }
private:
				void		Invalidate			();
				void		GetFootStepMatrix	( Fmatrix	&m, const Fmatrix &gl_anim, const  SIKCollideData &cld, bool collide );
inline				float		CollideFoot			( float angle, const Fmatrix &gl_anim, Fplane &p, Fvector3& ax );
inline				void		make_shift			(Fmatrix &xm, const Fplane &p,const Fvector3& pick_dir );
...
inline		static	void		get_start			( Fmatrix &start, SCalculateData &D, unsigned short bone );
private:
