...
*/
	...
	if (m_input_external_handler && !m_input_external_handler->authorized(dik))
	{
		return;
	}
	...
	if (m_holder && kUSE != dik)
	{
		...
		if (m_holder->allowWeapon( ) && inventory( ).Action(dik, CMD_START))
		{
			return;
		}
	...
	else if (inventory( ).Action(dik, CMD_START))
	{
		return;
	}
...
*/
	...
	if (g_Alive( ))
	{
		...
		if (m_holder)
		{
			...
			if (m_holder->allowWeapon( ) && inventory( ).Action(dik, CMD_STOP))
			{
				return;
			}
		...
		else if (inventory( ).Action(dik, CMD_STOP))
		{
			return;
		}
...
*/
	...
	switch (dik)
	{
		...
		case kDOWN:
		{
			cam_Active( )->Move((dik == kUP) ? kDOWN : kUP, 0, LookFactor);
		}
		break;
		case kCAM_ZOOM_IN:
		case kCAM_ZOOM_OUT:
		...
		case kRIGHT:
		{
			...
			{
				cam_Active( )->Move(dik, 0, LookFactor);
			}
		}
		break;
		case kACCEL:
