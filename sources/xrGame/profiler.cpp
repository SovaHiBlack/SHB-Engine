...
{
	inline bool operator()			(const SProfileResultPortion& _1, const SProfileResultPortion& _2) const
	{
...
void CProfiler::setup_timer(const char* timer_id, const unsigned __int64& timer_time, const u32& call_count)
{
	string256					m_temp;
	...
	if (i == m_timers.end( ))
	{
		...
		while ((j = strchr(k, '/')) != 0)
		{
			*j = 0;
			...
			if (m == m_timers.end( ))
				m_timers.insert(std::make_pair(CSharedString(m_temp), SProfileStats( )));
			*j = '/';
			k = j + 1;
		}
		i = m_timers.insert(std::make_pair(CSharedString(timer_id), SProfileStats( ))).first;
...
void CProfiler::show_stats(CGameFont* game_font, bool show)
{
	...
	if (!m_portions.empty( ))
	{
		...
		unsigned __int64						timer_time = 0;
		u32						call_count = 0;

		ProfileResultPortionsVec::const_iterator	I = m_portions.begin( ), J = I;
		ProfileResultPortionsVec::const_iterator	E = m_portions.end( );
		for (; I != E; ++I)
		{
