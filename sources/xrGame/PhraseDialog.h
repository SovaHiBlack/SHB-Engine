...
struct SPhraseDialogData : CSharedResource
{
	...
	//однонаправленый граф фраз - описывает все возможные варианты развития диалога
	CPhraseGraph	m_PhraseGraph;

	//список скриптовых предикатов, выполнение, которых необходимо для начала диалога
...
public:
	CPhrase* AddPhrase(pcstr text, const shared_str& phrase_id, const shared_str& prev_phrase_id, s32 goodwil_level);
	CPhrase* AddPhrase_script(pcstr text, pcstr phrase_id, pcstr prev_phrase_id, s32 goodwil_level)
	{
