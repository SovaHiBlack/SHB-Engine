...
struct CSaver
{
	...
	struct CHelper1
	{
		...
		template <>
		inline	static void save_data<true>(const T& data, M& stream, const P& p)
		{
	...
	inline	static void save_data(const char* data, M& stream, const P& p)
	{
		stream.w_stringZ(data);
	}
...
namespace object_saver
{
	namespace detail
	{
		...
		{
			template <typename T1, typename T2>
			inline	bool operator()	(const T1& data, const T2& value) const
			{
				return(true);
			}
			template <typename T1, typename T2>
			inline	bool operator()	(const T1& data, const T2& value, bool) const
			{
				return(true);
			}
		};
