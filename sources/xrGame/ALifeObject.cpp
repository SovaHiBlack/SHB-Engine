...
#pragma warning(disable:4238)
	CConfigurationFile ini(&IReader((void*) (ini_string), xr_strlen(ini_string)), FS.get_path("$game_config$")->m_Path);
#pragma warning(pop)
	...
	if (ini.section_exist("spawn"))
	{
		...
		const char* V;
		float p;
		for (unsigned int k = 0, j; ini.r_line("spawn", k, &N, &V); k++)
		{
			VERIFY(xr_strlen(N));
