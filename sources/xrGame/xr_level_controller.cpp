...
bool is_binded(EGameActions _action_id, s32 _dik)
{
...
s32 get_action_dik(EGameActions _action_id)
{
	_binding* pbinding = &g_key_bindings[_action_id];
...
EGameActions get_binded_action(s32 _dik)
{
	for (s32 idx = 0; idx < bindings_count; ++idx)
	{
...
void GetActionAllBinding(pcstr _action, pstr dst_buff, s32 dst_buff_sz)
{
	s32 action_id = action_name_to_id(_action);
...
public:
	CCC_Bind(pcstr N, s32 idx) : IConsoleCommand(N), m_work_idx(idx)
	{ }
...
public:
	CCC_UnBind(pcstr N, s32 idx) : IConsoleCommand(N), m_work_idx(idx)
	{
	...
	virtual void Execute(pcstr args)
	{
		s32 action_id = action_name_to_id(args);
		_binding* pbinding = &g_key_bindings[action_id];
...
{
public:
	CCC_ListActions(pcstr N) : IConsoleCommand(N)
	{
		bEmptyArgsHandled = true;
	}
...
{
public:
	CCC_UnBindAll(pcstr N) : IConsoleCommand(N)
	{
		bEmptyArgsHandled = true;
	}
...
{
public:
	CCC_BindList(pcstr N) : IConsoleCommand(N)
	{
		bEmptyArgsHandled = true;
	}
...
{
public:
	CCC_BindConsoleCmd(pcstr N) : IConsoleCommand(N)
	{ }
...
{
public:
	CCC_UnBindConsoleCmd(pcstr N) : IConsoleCommand(N)
	{
	...
	virtual void Execute(pcstr args)
	{
		s32 _dik = keyname_to_dik(args);
		bindConsoleCmds.unbind(_dik);
	}
};

void ConsoleBindCmds::bind(s32 dik, pcstr N)
{
	_conCmd& c = m_bindConsoleCmds[dik];
	c.cmd = N;
}
void ConsoleBindCmds::unbind(s32 dik)
{
	xr_map<s32, _conCmd>::iterator it = m_bindConsoleCmds.find(dik);
...
bool ConsoleBindCmds::execute(s32 dik)
{
	xr_map<s32, _conCmd>::iterator it = m_bindConsoleCmds.find(dik);
	if (it == m_bindConsoleCmds.end( ))
	{
...
{
	xr_map<s32, _conCmd>::iterator it = m_bindConsoleCmds.begin( );
