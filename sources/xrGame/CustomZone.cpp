...
#define FASTMODE_DISTANCE			(50.0f)			//расстояние до камеры от сферы зоны, когда зона переключается в режим быстрого обновления
...
CCustomZone::CCustomZone( )
{
	...
	for (s32 i = 0; i < eZoneStateMax; i++)
	{
		m_StateTime[i] = 0;
	}

	m_dwAffectFrameNum = 0;
	m_fArtefactSpawnProbability = 0.0f;
	m_fThrowOutPower = 0.0f;
	m_fArtefactSpawnHeight = 0.0f;
	m_fBlowoutWindPowerMax = m_fStoreWindPower = 0.0f;
...
void CCustomZone::shedule_Update(u32 dt)
{
	...
	if (IsEnabled( ))
	{
		...
		//пройтись по всем объектам в зоне и проверить их состояние
...
f32 CCustomZone::distance_to_center(CObject* O)
{
	fVector3 P;
...
void CCustomZone::OnMove( )
{
	...
	{
		f32 time_delta = f32(Device.dwTimeGlobal - m_dwLastTimeMoved) / 1000.0f;
		m_dwLastTimeMoved = Device.dwTimeGlobal;
...
void CCustomZone::SwitchZoneState(EZoneState new_state)
{
	if (OnServer( ))
	{
		// !!! Только одна запись для данного состояния !!!
		CNetPacket P;
...
void CCustomZone::BornArtefact( )
{
	...
	if (::Random.randF(0.0f, 1.0f) > m_fArtefactSpawnProbability)
	{
		return;
	}
