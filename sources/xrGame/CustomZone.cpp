...
//////////////////////////////////////////////////////////////////////////
	...
	if (m_zone_flags.test(eSpawnBlowoutArtefacts))
	{
		...
		unsigned short m_wItemCount = (unsigned short) _GetItemCount(l_caParameters);
		...
		for (unsigned short i = 0; i < m_wItemCount; ++i)
		{
...
void CCustomZone::PlayHitParticles(CGameObject* pObject)
{
	...
	if (particle_str.size( ))
	{
		...
		if (PP)
		{
			unsigned short play_bone = PP->GetRandomBone( );
...
void CCustomZone::OnEvent(CNetPacket& P, unsigned short type)
{
	switch (type)
	{
...
void CCustomZone::OnOwnershipTake(unsigned short id)
{
...
void CCustomZone::SwitchZoneState(EZoneState new_state)
{
	if (OnServer( ))
	{
		...
		P.w_u8(unsigned char(new_state));
		u_EventSend(P);
	};
...
void CCustomZone::CreateHit(unsigned short id_to,
	...
	ALife::EHitType hit_type)
{
	if (OnServer( ))
	{
		if (m_owner_id != u32(-1))
		{
			id_from = (unsigned short) m_owner_id;
		}
...
void CCustomZone::GoDisabledState( )
{
	...
	CNetPacket P;
	u_EventGen(P, GE_ZONE_STATE_CHANGE, ID( ));
	P.w_u8(unsigned char(eZoneStateDisabled));
	u_EventSend(P);
...
void CCustomZone::GoEnabledState( )
{
	...
	CNetPacket P;
	u_EventGen(P, GE_ZONE_STATE_CHANGE, ID( ));
	P.w_u8(unsigned char(eZoneStateIdle));
	u_EventSend(P);
}
