...
s32 dcTriListCollider::dSortedTriBox(
...
s32 dcTriListCollider::dTriBox(const dReal* v0, const dReal* v1, const dReal* v2,
							   Triangle* T,
							   dxGeom* o1, dxGeom* o2,
							   s32 flags, dContactGeom* contact, s32 skip
)
			...
			if(depth##ax>0.f){\
				if(depth##ax*1.05f<outDepth) \
					{\
						...
						for(s32 ii=0;ii<3;++ii) crpos[ii]=p[ii]+R[ii*4+ix1]*hside[ix1]*sgn1+R[ii*4+ix2]*hside[ix2]*sgn2;\
