...
void CWeaponKnife::Load(pcstr section)
{
	...
	SHudSound::LoadSound(section, "snd_shoot", m_sndShot, ESoundTypes(eST_WEAPON_SHOOTING));
...
void CWeaponKnife::KnifeStrike(const fVector3& pos, const fVector3& dir)
{
	...
	cartridge.m_impair = 1.0f;
	cartridge.m_kDisp = 1.0f;
...
void CWeaponKnife::LoadFireParams(pcstr section, pcstr prefix)
{
	...
	s32 num_game_diff_param = _GetItemCount(*s_sHitPower_2);//узнаём колличество параметров для хитов
