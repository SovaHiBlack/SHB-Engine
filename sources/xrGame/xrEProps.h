...
typedef fastdelegate::FastDelegate3<pcstr, pcstr, EItemType>	TOnItemRename;
typedef fastdelegate::FastDelegate3<pcstr, EItemType, bool&>	TOnItemRemove;
...
// Prepare Key
//------------------------------------------------------------------------------
IC xr_string FolderAppend(pcstr val)
{
	xr_string	tmp = (val && val[0]) ? val : "";
	if (val && val[0])tmp += "\\";
	return 	tmp;
}
IC shared_str PrepareKey(pcstr pref, pcstr key)
{
	R_ASSERT(key);
	return 		shared_str(xr_string(FolderAppend(pref) + key).c_str( ));
}
IC shared_str PrepareKey(pcstr pref0, pcstr pref1, pcstr key)
{
	R_ASSERT(key);
	return 		shared_str(xr_string(FolderAppend(pref0) + FolderAppend(pref1) + key).c_str( ));
}
IC shared_str PrepareKey(pcstr pref0, pcstr pref1, pcstr pref2, pcstr key)
{
	R_ASSERT(key);
...
public:
	...
	virtual CanvasValue* __stdcall	CreateCanvas(PropItemVec& items, shared_str key, shared_str val, s32 height) = 0;
	virtual ButtonValue* __stdcall	CreateButton(PropItemVec& items, shared_str key, shared_str val, u32 flags) = 0;
	virtual ChooseValue* __stdcall	CreateChoose(PropItemVec& items, shared_str key, shared_str* val, u32 mode, pcstr path = 0, pvoid fill_param = 0, u32 sub_item_count = 1, u32 choose_flags = cfAllowNone) = 0;
	...
	virtual FloatValue* __stdcall	CreateFloat(PropItemVec& items, shared_str key, f32* val, f32 mn = 0.f, f32 mx = 1.f, f32 inc = 0.01f, s32 decim = 2) = 0;
	virtual BOOLValue* __stdcall	CreateBOOL(PropItemVec& items, shared_str key, BOOL* val) = 0;
	virtual VectorValue* __stdcall	CreateVector(PropItemVec& items, shared_str key, fVector3* val, f32 mn = 0.f, f32 mx = 1.f, f32 inc = 0.01f, s32 decim = 2) = 0;
	...
	virtual FloatValue* __stdcall	CreateAngle(PropItemVec& items, shared_str key, f32* val, f32 mn = flt_min, f32 mx = flt_max, f32 inc = 0.01f, s32 decim = 2) = 0;
	virtual VectorValue* __stdcall	CreateAngle3(PropItemVec& items, shared_str key, fVector3* val, f32 mn = flt_min, f32 mx = flt_max, f32 inc = 0.01f, s32 decim = 2) = 0;
	...
	virtual CListValue* __stdcall	CreateCList(PropItemVec& items, shared_str key, pstr val, u32 sz, xr_string* lst, u32 cnt) = 0;
	virtual CTextValue* __stdcall	CreateCName(PropItemVec& items, shared_str key, pstr val, u32 sz, ListItem* owner) = 0;
...
public:
	virtual ListItem* __stdcall	CreateItem(ListItemsVec& items, pcstr key, s32 type, u32 item_flags = 0, pvoid object = 0) = 0;
};
