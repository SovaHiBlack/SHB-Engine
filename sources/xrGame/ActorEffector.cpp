...
{
	if (!CEffectorCam::Process(p, d, n, fFov, fFar, fAspect))
	{
		return FALSE;
	}
...
CControllerPsyHitCamEffector::CControllerPsyHitCamEffector(ECameraEffectorType type, const Fvector3& src_pos, const Fvector3& target_pos, float time) : inherited(eCEControllerPsyHit, flt_max)
{
	m_time_total = time;
	m_time_current = 0.0f;
...
const float	_base_fov = 170.0f;
const float	_max_fov_add = 160.0f;
