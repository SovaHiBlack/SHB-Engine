...
{
	// sounds
	SHudSound::DestroySound(sndShotG);
	SHudSound::DestroySound(sndReloadG);
	SHudSound::DestroySound(sndSwitch);
}

void CWeaponMagazinedWGrenade::StopHUDSounds( )
{
	SHudSound::StopSound(sndShotG);
	SHudSound::StopSound(sndReloadG);
	SHudSound::StopSound(sndSwitch);
...
void CWeaponMagazinedWGrenade::Load(pcstr section)
{
	...
	//// Sounds
	SHudSound::LoadSound(section, "snd_shoot_grenade", sndShotG, m_eSoundShot);
	SHudSound::LoadSound(section, "snd_reload_grenade", sndReloadG, m_eSoundReload);
	SHudSound::LoadSound(section, "snd_switch", sndSwitch, m_eSoundReload);
...
void CWeaponMagazinedWGrenade::PerformSwitchGL( )
{
	...
	iAmmoElapsed = (s32)m_magazine.size( );
...
void CWeaponMagazinedWGrenade::PlayAnimModeSwitch( )
{
	if (m_bGrenadeMode)
	{
		m_pHUD->animPlay(random_anim(mhud_switch_g), FALSE, this, eSwitch);
	} //fake
	else
	{
		m_pHUD->animPlay(random_anim(mhud_switch), FALSE, this, eSwitch);
	} //fake
}
