...
C_StepManager::C_StepManager( )
{ }

C_StepManager::~C_StepManager( )
{ }

DLL_Pure* C_StepManager::_construct( )
{
...
void C_StepManager::reload(const char* section)
{
	...
	for (u32 i = 0; pSettings->r_line(anim_section, i, &anim_name, &val); ++i)
	{
		...
		param.cycles = unsigned char(atoi(cur_elem));
...
void C_StepManager::on_animation_start(MotionID motion_id, CBlend* blend)
{
	m_blend = blend;
	if (!m_blend)
	{
	...
	// искать текущую анимацию в StepsMap
	StepsMap_it it = m_steps_map.find(motion_id);
...
void C_StepManager::update( )
{
	...
	S_StepParam& step = m_step_info.params;
	...
	for (u32 i = 0; i < m_legs_count; i++)
	{
		...
		if (offset_time <= cur_time)
		{
			...
			if (!mtl_pair->CollideParticles.empty( ))
			{
				...
				// установить позицию
				pos.c.set(get_foot_position(E_LegType(i)));
	...
	{
		m_step_info.cur_cycle = 1 + unsigned char(float(cur_time - m_time_anim_started) / (1000.0f * cycle_anim_time));
	}
...
//////////////////////////////////////////////////////////////////////////
Fvector3 C_StepManager::get_foot_position(E_LegType leg_type)
{
...
void C_StepManager::load_foot_bones(CConfigurationFile::Sect& data)
{
	for (CConfigurationFile::SectCIt I = data.Data.begin( ); I != data.Data.end( ); ++I)
	{
		...
		{
			m_foot_bones[eLT_FrontLeft] = index;
		}
		else if (xr_strcmp(*item.first, "front_right") == 0)
		{
			m_foot_bones[eLT_FrontRight] = index;
		}
		else if (xr_strcmp(*item.first, "back_right") == 0)
		{
			m_foot_bones[eLT_BackRight] = index;
		}
		else if (xr_strcmp(*item.first, "back_left") == 0)
		{
			m_foot_bones[eLT_BackLeft] = index;
		}
	}
}
...
float C_StepManager::get_blend_time( )
{
