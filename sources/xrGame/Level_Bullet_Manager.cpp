...
{
	m_Lock.Enter	();
	VERIFY		(unsigned short(-1)!=cartridge.bullet_material_idx);
	...
	for(int k=m_Bullets.size()-1; k>=0; k--){
		...
		for(u32 i=0; i<cur_step_num; i++){
			if(!CalcBullet(rq_storage,rq_spatial,&bullet, m_dwStepTime)){
				...
				RegisterEvent(EVENT_REMOVE, FALSE, &bullet, Fvector3().set(0, 0, 0), res, (unsigned short)k);
...
void CBulletManager::RegisterEvent			(EventType Type, BOOL _dynamic, SBullet* bullet, const Fvector3& end_point, collide::rq_result& R, unsigned short tgt_material)
{
