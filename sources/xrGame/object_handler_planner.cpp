...
#ifdef LOG_ACTION
pcstr CObjectHandlerPlanner::action2string(const _action_id_type& id)
{
...
pcstr CObjectHandlerPlanner::property2string(const _condition_type& id)
{
	pstr S = m_temp_string;
	if (action_object_id(id) != 0xffff)
		if (Level( ).Objects.net_Find(action_object_id(id)))
	...
	switch (action_state_id(id))
	{
		...
		{
			strcat(S, "IdleStrap");			break;
		}
		case ObjectHandlerSpace::eWorldPropertyDropped:
		{
			strcat(S, "Dropped");				break;
		}
		...
		{
			strcat(S, "QueueWait2");			break;
		}
		case ObjectHandlerSpace::eWorldPropertyThrowStarted:
		{
			strcat(S, "ThrowStarted");		break;
		}
		...
		{
			strcat(S, "UseEnough");			break;
		}
		case ObjectHandlerSpace::eWorldPropertyItemID:
		{
			S[xr_strlen(S) - 1] = 0;			break;
		}
		default: NODEFAULT;
	}
	return		(S);
