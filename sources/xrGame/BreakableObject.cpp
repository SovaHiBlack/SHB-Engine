...
float	CBreakableObject::m_damage_threshold = 5.0f;
float	CBreakableObject::m_health_threshhold = 0.0f;
...
void CBreakableObject::Break( )
{
	...
	CreateBroken( );
	ActivateBroken( );
	unsigned short el_num = m_pPhysicsShell->get_ElementsNumber( );
	for (unsigned short i = 0; i < el_num; i++)
	{
		Fvector3 pos;
		Fvector3 dir;
...
void CBreakableObject::ApplyExplosion(const Fvector3& dir, float impulse)
{
	...
	for (unsigned short i = 0; i < el_num; i++)
	{
		...
		float sign = max_area_dir.dotproduct(dir) > 0.0f ? 1.0f : -1.0f;
