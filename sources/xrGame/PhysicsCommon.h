...
IC f32 Erp(f32 k_p, f32 k_d, f32 s = fixed_step)
{
	return ((s * (k_p)) / (((s) * (k_p)) + (k_d)));
}
IC f32 Cfm(f32 k_p, f32 k_d, f32 s = fixed_step)
{
	return (1.0f / (((s) * (k_p)) + (k_d)));
}
IC f32 Spring(f32 cfm, f32 erp, f32	s = fixed_step)
{
	return ((erp) / (cfm) / s);
}
IC f32 Damping(f32 cfm, f32 erp)
{
	return ((1.0f - (erp)) / (cfm));
}
IC void	 MulSprDmp(f32& cfm, f32& erp, f32 mul_spring, f32 mul_damping)
{
	f32 factor = 1.0f / (mul_spring * erp + mul_damping * (1 - erp));
	cfm *= factor;
