// relation_registry_inline.h:	реестр для хранения данных об отношении персонажа к другим персонажам
...
ALife::ERelationType SRelationRegistry::GetRelationType(T from, T to) const
{
	...
	CharacterGoodwill attitude = GetAttitude(from, to);
...
template<typename T>
CharacterGoodwill SRelationRegistry::GetAttitude(T from, T to) const
{
	// личное отношение from к to
	CharacterGoodwill presonal_goodwill = GetGoodwill(from->object_id( ), to->object_id( )); VERIFY(presonal_goodwill != NO_GOODWILL);
	// влияние репутации персонажей
	CharacterGoodwill reputation_goodwill = GetReputationRelation(from->Reputation( ), to->Reputation( ));
	// влияние рангов персонажей
	CharacterGoodwill rank_goodwill = GetRankRelation(from->Rank( ), to->Rank( ));
	// отношение группировки from персонально к to
	CharacterGoodwill community_goodwill = GetCommunityGoodwill(from->Community( ), to->object_id( )); VERIFY(community_goodwill != NO_GOODWILL);
	// отношение группировки from к группировки to
	CharacterGoodwill community_to_community = GetCommunityRelation(from->Community( ), to->Community( ));

	CharacterGoodwill attitude = presonal_goodwill +
		reputation_goodwill +
		rank_goodwill +
		community_goodwill +
