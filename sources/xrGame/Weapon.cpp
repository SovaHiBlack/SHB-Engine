...
void CWeapon::UpdateFireDependencies_internal( )
{
	if (Device.dwFrame != dwFP_Frame)
	{
		...
		if (GetHUDmode( ) && (0 != H_Parent( )))
		{
			...
			m_firedeps.vLastFD.set(0.0f, 0.0f, 1.0f);
			...
			fVector3::generate_orthonormal_basis_normalized(m_firedeps.m_FireParticlesXForm.k, m_firedeps.m_FireParticlesXForm.j, m_firedeps.m_FireParticlesXForm.i);
		}
		else
		{
...
void CWeapon::Load(pcstr section)
{
	...
	string256 temp;
	for (s32 i = egdNovice; i < egdCount; ++i)
	{
...
//			u8 NewAmmoType = 
			...
			if (OnClient( ))
			{
				SetAmmoElapsed(s32(AmmoElapsed));
			}
...
s32 CWeapon::GetAmmoCurrent(bool use_item_to_spawn) const
{
	s32 l_count = iAmmoElapsed;
	...
	for (s32 i = 0; i < (s32)m_ammoTypes.size( ); ++i)
	{
...
f32 CWeapon::GetConditionMisfireProbability( ) const
{
	...
	f32 mis = misfireProbability + powf(1.0f - GetCondition( ), 3.0f) * misfireConditionK;
...
BOOL CWeapon::CheckForMisfire( )
{
	...
	f32 rnd = ::Random.randF(0.0f, 1.0f);
	f32 mp = GetConditionMisfireProbability( );
	if (rnd < mp)
	{
		FireEnd( );
...
void CWeapon::modify_holder_params(f32& range, f32& fov) const
{
	if (!IsScopeAttached( ))
	{
...
f32 CWeapon::Weight( )
{
	f32 res = CInventoryItemObject::Weight( );
	...
	if (iAmmoElapsed)
	{
		f32 w = pSettings->r_float(*m_ammoTypes[m_ammoType], "inv_weight");
		f32 bs = pSettings->r_float(*m_ammoTypes[m_ammoType], "box_size");
...
f32 CWeapon::GetConditionToShow( ) const
{
	return GetCondition( );
}
...
const f32& CWeapon::hit_probability( ) const
{
