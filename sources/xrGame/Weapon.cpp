...
#define WEAPON_REMOVE_TIME_DEFAULT		60000
...
CWeapon::CWeapon(const char* name)
{
	SetState(eHidden);
...
void CWeapon::Load(const char* section)
{
	...
	//шаг смещения ствола в горизонтальной плоскости в момент выстрела
	...
	{
		m_dwWeaponRemoveTime = WEAPON_REMOVE_TIME_DEFAULT;
	}
	//////////////////////////////////////
	...
	m_u8TracerColorID = READ_IF_EXISTS(pSettings, r_u8, section, "tracers_color_ID", unsigned char(-1));
...
/*
void CWeapon::animGet	(MotionSVec& lst, const char* prefix)
{
...
BOOL CWeapon::net_Spawn(CSE_Abstract* DC)
{
	...
	m_DefaultCartridge.Load(*m_ammoTypes[m_ammoType], unsigned char(m_ammoType));
	if (iAmmoElapsed)
	{
...
void CWeapon::net_Export(CNetPacket& P)
{
	...
	P.w_u8((unsigned char) m_bZoomMode);
}
...
//			unsigned char NewAmmoType = 
			P.r_u8( );
			unsigned char AmmoElapsed = P.r_u8( );
			unsigned char NextAmmo = P.r_u8( );
			if (NextAmmo == unsigned char(-1))
			{
				m_set_next_ammoType_on_reload = u32(-1);
			}
			else
			{
				m_set_next_ammoType_on_reload = unsigned char(NextAmmo);
			}
...
int CWeapon::GetAmmoCurrent(bool use_item_to_spawn) const
{
	...
	{
		const char* l_ammoType = *m_ammoTypes[i];
...
float CWeapon::GetConditionMisfireProbability( ) const
{
	...
	float mis = misfireProbability + powf(1.0f - GetCondition( ), 3.0f) * misfireConditionK;
...
const char* wpn_scope = "wpn_scope";
const char* wpn_silencer = "wpn_silencer";
const char* wpn_grenade_launcher = "wpn_grenade_launcher";
const char* wpn_launcher = "wpn_launcher";
...
void CWeapon::SwitchState(u32 S)
{
	...
	{	// !!! Just single entry for given state !!!
		...
		P.w_u8(unsigned char(S));
		P.w_u8(unsigned char(m_sub_state));
		P.w_u8(unsigned char(m_ammoType & 0xff));
		P.w_u8(unsigned char(iAmmoElapsed & 0xff));
		P.w_u8(unsigned char(m_set_next_ammoType_on_reload & 0xff));
...
CInventoryItem* CWeapon::can_kill(CInventory* inventory) const
{
	...
	for (; I != E; ++I)
	{
		...
		xr_vector<CSharedString>::const_iterator	i = std::find(m_ammoTypes.begin( ), m_ammoTypes.end( ), inventory_item->object( ).cNameSect( ));
...
void CWeapon::UpdateHudAdditonal(Fmatrix& trans)
{
	...
	if ((pActor->IsZoomAimingMode( ) && m_fZoomRotationFactor <= 1.0f) || (!pActor->IsZoomAimingMode( ) && m_fZoomRotationFactor > 0.0f))
	{
...
void CWeapon::SetAmmoElapsed(int ammo_count)
{
	...
	if (uAmmo != m_magazine.size( ))
	{
		if (uAmmo > m_magazine.size( ))
		{
			...
			l_cartridge.Load(*m_ammoTypes[m_ammoType], unsigned char(m_ammoType));
...
bool CWeapon::IsNecessaryItem(const CSharedString& item_sect)
{
...
const char* CWeapon::GetCurrentAmmo_ShortName( )
{
	if (m_magazine.empty( ))
	{
		return ("");
