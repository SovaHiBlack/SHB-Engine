	...
	m_interactive_motion(0)
{
	...
	m_shot_up_factor = 0.0f;
	m_after_death_velocity_factor = 1.0f;
	m_ik_controller = NULL;
	m_BonceDamageFactor = 1.0f;
...
bool is_similar(const fMatrix4x4& m0, const fMatrix4x4& m1, f32 param)
{
	fMatrix4x4 tmp1;
	tmp1.invert(m0);
	fMatrix4x4 tmp2;
...
void CCharacterPhysicsSupport::KillHit(CObject* who, ALife::EHitType hit_type, f32& impulse)
{
	...
	if (!m_was_wounded)
	{
		impulse *= (hit_type == ALife::eHitTypeExplosion ? 1.0f : skel_fatal_impulse_factor);
	}
...
void CCharacterPhysicsSupport::in_Hit(f32 P, fVector3& dir, CObject* who, s16 element, fVector3 p_in_object_space, f32 impulse, ALife::EHitType hit_type, bool is_killing)
{
	if (m_EntityAlife.use_simplified_visual( ))
	{
		return;
	...
	if (m_EntityAlife.g_Alive( ) && is_killing && hit_type == ALife::eHitTypeExplosion && P > 70.0f)
	{
...
void CCharacterPhysicsSupport::CreateSkeleton( )
{
	...
	m_pPhysicsShell->set_JointResistance(0.0f);
...
void CCharacterPhysicsSupport::CollisionCorrectObjPos(const fVector3& start_from, bool character_create/*=false*/)
{
	...
	activation_shape.Activate(vbox, 1, 1.0f, M_PI / 8.0f);
...
void CCharacterPhysicsSupport::ActivateShell(CObject* who)
{
	...
	if (anim_mov_ctrl && anim_mov_blend && anim_mov_blend->blend != CBlend::eFREE_SLOT && anim_mov_blend->timeCurrent + Device.fTimeDelta * anim_mov_blend->speed < anim_mov_blend->timeTotal - SAMPLE_SPF - EPSILON_5)//.
	{
...
void CCharacterPhysicsSupport::UpdateFrictionAndJointResistanse( )
{
	...
	f32 curr_joint_resistance = hinge_force_factor1 - (skel_remain_time * hinge_force_factor1) / skel_ddelay;
