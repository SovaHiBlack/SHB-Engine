...
float CStalker::GetWeaponAccuracy	() const
{
	float				base = PI/180.f;
...
void CStalker::g_WeaponBones	(int &L, int &R1, int &R2)
{
...
void CStalker::HitSignal				(float amount, Fvector3& vLocalDir, CObject* who, signed short element)
{
	if (getDestroy())
		return;
...
void CStalker::update_range_fov		(float &new_range, float &new_fov, float start_range, float start_fov)
{
...
float CStalker::missile_throw_force		() 
{
	update_throw_params		();
