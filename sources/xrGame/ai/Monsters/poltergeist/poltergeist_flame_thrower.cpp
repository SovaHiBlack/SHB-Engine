...
void CPolterFlame::load(pcstr section)
{
	...
	// load scan effector
	pcstr ppi_section = pSettings->r_string(section, "flame_scan_effector_section");
				...
				if (elem->time_last_hit + m_hit_delay < time()) {
					...
					if (Level().ObjectSpace.RayPick(elem->position, elem->target_dir, m_length, collide::rqtBoth, rq, NULL)) {
						if ((rq.O == elem->target_object) && (rq.range < m_length)) {
							...
							HS.p_in_bone_space	= (fVector3().set(0.0f,0.0f,0.0f));			//					P.w_vec3		(fVector3().set(0.0f,0.0f,0.0f));
