...
#define EFFECTOR_ID_GEN(type) (type( u32(unsigned __int64(this) & u32(-1)) ))
...
BOOL CAlienEffector::Process(Fvector3& p, Fvector3& d, Fvector3& n, float& fFov, float& fFar, float& fAspect)
{
	...
	//set fov
	float	rel_speed = monster->m_fCurSpeed / 15.0f;
	clamp(rel_speed, 0.0f, 1.1f);
