...
CVampirePPEffector::CVampirePPEffector(const SPPInfo& ppi, f32 life_time) : inherited(EEffectorPPType(eCEHit), life_time)
{
	state = ppi;
	m_total = life_time;
}
...
CVampireCameraEffector::CVampireCameraEffector(f32 time, const fVector3& src, const fVector3& tgt) : inherited(eCEVampire, time)
{
	fLifeTime = time;
	m_time_total = time;
...
BOOL CVampireCameraEffector::Process(fVector3& p, fVector3& d, fVector3& n, f32& fFov, f32& fFar, f32& fAspect)
{
	...
	{
		dangle_target.x = 0.0f;
		dangle_target.y = 0.0f;
		dangle_target.z = 0.0f;
