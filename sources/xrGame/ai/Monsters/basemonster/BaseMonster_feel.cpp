...
void CBaseMonster::HitEntity(const CEntity *pEntity, float fDamage, float impulse, Fvector3& dir)
{
	...
	if (EnemyMan.get_enemy() == pEntity) {
		...
		if (pEntityNC == Actor()) {
			...
			if(!ce)
			{
				const CSharedString&	eff_sect = pSettings->r_string(cNameSect(), "actor_hit_effect");
				if(eff_sect.c_str())
				{
					int id						= -1;
					Fvector3						cam_pos;
...
void CBaseMonster::HitSignal(float amount, Fvector3& vLocalDir, CObject* who, signed short element)
{
	if (!g_Alive( ))
	{
		return;
