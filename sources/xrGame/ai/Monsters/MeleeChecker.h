#pragma once

class CBaseMonster;
class CEntity;

#define HIT_STACK_SIZE						2

class CMeleeChecker
{
private:
	collide::rq_results						r_res;

	CBaseMonster*							m_object;

	// ltx parameters
	float									m_min_attack_distance;
	float									m_max_attack_distance;
	float									m_as_min_dist;
	float									m_as_step;

	bool									m_hit_stack[HIT_STACK_SIZE];

	float									m_current_min_distance;

public:
	void			init_external			(CBaseMonster* obj)
	{
		m_object = obj;
	}
	inline void		load					(const char* section);

	// инициализировано состо€ние атаки
	inline void		init_attack				( );
	void			on_hit_attempt			(bool hit_success);

	// ѕолучить рассто€ние от fire_bone до врага
	// ¬ыполнить RayQuery от fire_bone в enemy.center
	float			distance_to_enemy		(const CEntity* enemy);

	inline float	get_min_distance		( );
	inline float	get_max_distance		( );

	inline bool		can_start_melee			(const CEntity* enemy);
	inline bool		should_stop_melee		(const CEntity* enemy);

#ifdef DEBUG
	inline float	dbg_as_min_dist			( )
	{
		return m_as_min_dist;
	}
	inline float	dbg_as_step				( )
	{
		return m_as_step;
	}
#endif // def DEBUG

};

#include "MeleeChecker_inline.h"
