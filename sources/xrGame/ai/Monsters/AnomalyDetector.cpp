...
{
	m_radius = READ_IF_EXISTS(pSettings, r_float, section, "Anomaly_Detect_Radius", 15.0f);
...
void CAnomalyDetector::update_schedule( )
{
	...
	xr_vector<unsigned short>			temp_out_restrictors;
	xr_vector<unsigned short>			temp_in_restrictors;

	temp_in_restrictors.reserve(m_storage.size( ));

	// add new restrictions
	for (AnomalyInfoVec_it it = m_storage.begin( ); it != m_storage.end( ); it++)
	{
...
void CAnomalyDetector::on_contact(CObject* obj)
{
	...
	if (custom_zone->restrictor_type( ) == Restriction::eRestrictorTypeNone)
	{
		return;
	}
	...
	AnomalyInfoVec_it it = std::find(m_storage.begin( ), m_storage.end( ), custom_zone);
	if (it != m_storage.end( ))
	{
		return;
	}
