...
bool CStateBloodsuckerPredatorAbstract::check_completion( )
{
	...
	if (object->EnemyMan.get_enemy( ) && object->EnemyMan.see_enemy_now( ) && (object->Position( ).distance_to(object->EnemyMan.get_enemy( )->Position( )) < 4.0f))
	{
		return true;
	}
...
void CStateBloodsuckerPredatorAbstract::setup_substates( )
{
	...
	if (current_substate == eStatePredator_MoveToCover)
	{
		...
		data.completion_dist = 0.0f;		// get exactly to the point
	...
	if (current_substate == eStatePredator_LookOpenPlace)
	{
		...
		data.point.mad(object->Position( ), dir, 10.0f);
...
void CStateBloodsuckerPredatorAbstract::select_camp_point( )
{
	...
	if (m_target_node == u32(-1))
	{
		const CCoverPoint* point = object->CoverMan->find_cover(object->Position( ), 10.0f, 30.0f);
		if (point)
		{
