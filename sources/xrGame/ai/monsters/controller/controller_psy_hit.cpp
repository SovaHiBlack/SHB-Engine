...
void CControllerPsyHit::death_glide_end()
{
	...
	monster->m_sound_tube_hit_left.play_at_pos(Actor(), Fvector3().set(-1.f, 0.f, 1.f), sm_2D);
	monster->m_sound_tube_hit_right.play_at_pos(Actor(), Fvector3().set(1.f, 0.f, 1.f), sm_2D);
...
void CControllerPsyHit::update_frame()
{
	...
	//		m_sound_state = ePull;
	//		monster->m_sound_tube_pull.play_at_pos(Actor(), Fvector3().set(0.f, 0.f, 0.f), sm_2D);
	//	}
	//}
}
...
void CControllerPsyHit::set_sound_state(ESoundState state)
{
	CController *monster = smart_cast<CController *>(m_object);
	if (state == ePrepare) {
		monster->m_sound_tube_prepare.play_at_pos(Actor(), Fvector3().set(0.f, 0.f, 0.f), sm_2D);
	} else 
	if (state == eStart) {
		if (monster->m_sound_tube_prepare._feedback())	monster->m_sound_tube_prepare.stop();

		monster->m_sound_tube_start.play_at_pos(Actor(), Fvector3().set(0.f, 0.f, 0.f), sm_2D);
		monster->m_sound_tube_pull.play_at_pos(Actor(), Fvector3().set(0.f, 0.f, 0.f), sm_2D);
	} else 
	if (state == eHit) {
