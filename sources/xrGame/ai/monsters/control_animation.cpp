...
void CControlAnimation::play_part(SAnimationPart& part, PlayCallback callback)
{
	...
	unsigned short bone_or_part = m_skeleton_animated->LL_GetMotionDef(part.motion)->bone_or_part;
	if (bone_or_part == unsigned short(-1))
	{
...
///////////////////////////////////////////////////////////////////////////////
	...
	if ((part.motion != m_data.torso.motion) && part.blend)
		m_object->C_StepManager::on_animation_start(part.motion, part.blend);


	AnimationsEventMap_it it = m_anim_events.find(part.motion);
	if (it != m_anim_events.end( ))
	{
		for (AnimationsEventVec_it event_it = it->second.begin( ); event_it != it->second.end( ); ++event_it)
		{
...
void CControlAnimation::add_anim_event(MotionID motion, float time_perc, u32 id)
{
	...
	AnimationsEventMap_it it = m_anim_events.find(motion);
	if (it != m_anim_events.end( ))
	{
		AnimationsEventVec& anim_vec = it->second;

		for (AnimationsEventVec_it I = anim_vec.begin( ); I != anim_vec.end( ); ++I)
		{
...
void CControlAnimation::freeze( )
{
	...
	if (m_data.global.blend)
	{
		m_saved_global_speed = m_data.global.blend->speed;	m_data.global.blend->speed = 0.f;
	}

	if (m_data.legs.blend)
	{
		m_saved_legs_speed = m_data.legs.blend->speed;		m_data.legs.blend->speed = 0.f;
	}

	if (m_data.torso.blend)
	{
		m_saved_torso_speed = m_data.torso.blend->speed;	m_data.torso.blend->speed = 0.f;
	}
}
