...
void CAttachmentOwner::reload(const char* section)
{
	...
	unsigned int item_count = _GetItemCount(attached_sections);
	char current_item_section[256];
...
CAttachableItem* CAttachmentOwner::attachedItem(unsigned short id) const
{
	xr_vector<CAttachableItem*>::const_iterator I = m_attached_objects.begin( );
	xr_vector<CAttachableItem*>::const_iterator E = m_attached_objects.end( );
	for (; I != E; ++I)
