...
CPatternFunction::CPatternFunction(pcstr caFileName, CEF_Storage* storage) : CBaseFunction(storage)
{
	m_dwPatternCount = 0;
	m_dwVariableCount = 0;
...
void CPatternFunction::vfLoadEF(pcstr caFileName)
{
	string_path		caPath;
	...
	for (i = 0; i < m_dwPatternCount; ++i)
	{
		...
		for (s32 j = 0; j < (s32)m_tpPatterns[i].dwCardinality; ++j)
...
f32 CPatternFunction::ffGetValue( )
{
	...
	if (psAI_Flags.test(aiFuncs))
	{
		...
		s32 j = sprintf_s(caString, sizeof(caString), "%32s (", m_caName);
