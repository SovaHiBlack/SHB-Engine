...
CALifeUpdateManager::CALifeUpdateManager(xrServer* server, pcstr section) :
...
//	prepare_objects_for_save		();
	...
	pcstr							temp0 = strstr(**m_server_command_line, "/");
	VERIFY(temp0);
	string256						temp;
...
void CALifeUpdateManager::load(pcstr game_name, bool no_assert, bool new_only)
{
...
void CALifeUpdateManager::reload(pcstr section)
{
	CALifeSimulatorBase::reload(section);
	set_process_time((s32)m_max_process_time);
	objects_per_update(m_objects_per_update);
}

bool CALifeUpdateManager::load_game(pcstr game_name, bool no_assert)
{
	{
...
void CALifeUpdateManager::jump_to_level(pcstr level_name) const
{
