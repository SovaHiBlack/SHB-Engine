...
{
	// sounds
	SHudSound::DestroySound(sndShow);
	SHudSound::DestroySound(sndHide);
	SHudSound::DestroySound(sndShot);
	SHudSound::DestroySound(sndEmptyClick);
	SHudSound::DestroySound(sndReload);
}

void CWeaponMagazined::StopHUDSounds( )
{
	SHudSound::StopSound(sndShow);
	SHudSound::StopSound(sndHide);

	SHudSound::StopSound(sndEmptyClick);
	SHudSound::StopSound(sndReload);

	SHudSound::StopSound(sndShot);
...
void CWeaponMagazined::Load(pcstr section)
{
	...
	// Sounds
	SHudSound::LoadSound(section, "snd_draw", sndShow, m_eSoundShow);
	SHudSound::LoadSound(section, "snd_holster", sndHide, m_eSoundHide);
	SHudSound::LoadSound(section, "snd_shoot", sndShot, m_eSoundShot);
	SHudSound::LoadSound(section, "snd_empty", sndEmptyClick, m_eSoundEmptyClick);
	SHudSound::LoadSound(section, "snd_reload", sndReload, m_eSoundReload);
	...
	if (m_eSilencerStatus == ALife::eAddonAttachable)
	{
		...
		SHudSound::LoadSound(section, "snd_silncer_shot", sndSilencerShot, m_eSoundShot);
	}
	...
	if (pSettings->line_exist(section, "fire_modes"))
	{
		...
		s32 ModesCount = _GetItemCount(FireModesList.c_str( ));
		m_aFireModes.clear( );
		for (s32 i = 0; i < ModesCount; i++)
		{
			string16 sItem;
			_GetItem(FireModesList.c_str( ), i, sItem);
			s32 FireMode = atoi(sItem);
...
void CWeaponMagazined::state_Fire(f32 dt)
{
...
void CWeaponMagazined::state_Misfire(f32 /**dt/**/)
{
	OnEmptyClick( );
	SwitchState(eIdle);
...
void CWeaponMagazined::state_MagEmpty(f32 dt)
{ }
...
void CWeaponMagazined::SetQueueSize(s32 size)
{
	m_iQueueSize = size;
	if (m_iQueueSize == -1)
	{
...
f32	CWeaponMagazined::GetWeaponDeterioration( )
{
...
{
	s32	AE = GetAmmoElapsed( );
	s32	AC = GetAmmoCurrent( );
