...
void CMainMenu::ReadTextureInfo( )
{
	if (pSettings->section_exist("texture_desc"))
	{
		...
		s32 itemsCount = _GetItemCount(itemsList.c_str( ));

		for (s32 i = 0; i < itemsCount; i++)
		{
...
void CMainMenu::IR_OnMousePress(s32 btn)
{
	if (!IsActive( ))
	{
		return;
...
void CMainMenu::IR_OnMouseWheel(s32 direction)
{
	if (!IsActive( ))
	{
		return;
...
void CMainMenu::Screenshot(IRender_interface::ScreenshotMode mode, pcstr name)
{
...
void CMainMenu::OnSessionTerminate(pcstr reason)
{
