...
//	_DECLARE_FUNCTION11	(SetSatiety,		void, float);
	...
	void				ChangeTeam(unsigned char team, unsigned char squad, unsigned char group);
	...
	void				set_home(const char* name, float r_min, float r_max, bool aggressive);
	void				remove_home( );
	void				berserk( );
	...
	//работа с заданиями
	ETaskState			GetGameTaskState(const char* task_id, int objective_num);
	void				SetGameTaskState(ETaskState state, const char* task_id, int objective_num);
