...
bool CPhraseDialog::SayPhrase(DIALOG_SHARED_PTR& phrase_dialog, const shared_str& phrase_id)
{
	...
	//сообщить CDialogManager, что сказана фраза и ожидается ответ
	if (first_is_speaking)
	{
...
void CPhraseDialog::load_shared(pcstr)
{
	...
	s32 phrase_num = pXML->GetNodesNum(phrase_list_node, "phrase");
...
void CPhraseDialog::SetPriority(s32 val)
{
	data( )->m_iPriority = val;
}

CPhrase* CPhraseDialog::AddPhrase(pcstr text, const shared_str& phrase_id, const shared_str& prev_phrase_id, s32 goodwil_level)
{
	CPhrase* phrase = NULL;
	...
	if (prev_phrase_id != "")
	{
		data( )->m_PhraseGraph.add_edge(prev_phrase_id, phrase_id, 0.0f);
	}
...
void CPhraseDialog::AddPhrase(CUIXml* pXml, XML_NODE* phrase_node, const shared_str& phrase_id, const shared_str& prev_phrase_id)
{
	...
	s32 gw = pXml->ReadInt(phrase_node, "goodwill", 0, -10000);
	...
	s32 next_num = pXml->GetNodesNum(phrase_node, "next");
	for (s32 i = 0; i < next_num; ++i)
	{
