...
static void show_restrictions(pcstr restrictions)
{
	string256			temp;
...
void CPatrolPathManager::set_previous_point(s32 point_index)
{
	if (!m_path)
	{
		ai( ).script_engine( ).script_log(eLuaMessageTypeError, "Path not specified (object %s)!", *m_game_object->cName( ));
...
void CPatrolPathManager::set_start_point(s32 point_index)
{
	if (!m_path)
	{
		ai( ).script_engine( ).script_log(eLuaMessageTypeError, "Path not specified (object %s)!", *m_game_object->cName( ));
