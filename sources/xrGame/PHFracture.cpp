...
void CPHFracturesHolder::InitNewElement(CPHElement* element, const fMatrix4x4& shift_pivot, f32 density)
{
...
void CPHFracturesHolder::PhTune(dBodyID body)
{
	...
	s32 num = dBodyGetNumJoints(body);
	for (s32 i = 0; i < num; ++i)
	{
...
{
	FRACTURE_I f_i = m_fractures.begin( );
	FRACTURE_I f_e = m_fractures.end( );
...
bool CPHFracture::Update(CPHElement* element)
{
	...
	gravity_force.set(0.0f, -ph_world->Gravity( ) * m_firstM.mass, 0.0f);
