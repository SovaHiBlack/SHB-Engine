//	Module 		: ALifeMonsterPatrolPathManager.cpp
...
CALifeMonsterPatrolPathManager::CALifeMonsterPatrolPathManager(object_type* object)
{
	...
	m_current_vertex_index = unsigned int(-1);
	m_previous_vertex_index = unsigned int(-1);
	m_start_vertex_index = unsigned int(-1);
...
const unsigned int& CALifeMonsterPatrolPathManager::target_level_vertex_id( ) const
{
...
void CALifeMonsterPatrolPathManager::navigate( )
{
	...
	using EDGES = CPatrolPath::CVertex::EDGES;
	...
	unsigned int chosen = use_randomness( ) ? object( ).randI(branching_factor) : 0;
