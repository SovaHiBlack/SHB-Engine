...
void CGameObject::OnEvent(CNetPacket& P, unsigned short type)
{
	switch (type)
	{
		case GE_HIT:
...
#pragma warning(disable:4238)
		m_ini_file = xr_new<CConfigurationFile>(&IReader((void*) (*(O->m_ini_string)), O->m_ini_string.size( )), FS.get_path("$game_config$")->m_Path);
#pragma warning(pop)
	}
...
void CGameObject::u_EventGen(CNetPacket& P, u32 type, u32 dest)
{
	...
	P.w_u16(unsigned short(type & 0xffff));
	P.w_u16(unsigned short(dest & 0xffff));
}
