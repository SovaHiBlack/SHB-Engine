...
bool CCoverEvaluatorBase::inertia						(f32 radius)
{
//	m_actuality				= m_actuality && fsimilar(m_last_radius,radius);
//	m_actuality				= m_actuality && ((m_last_radius + EPSILON_3) >= radius);
	bool					radius_criteria = ((m_last_radius + EPSILON_3) >= radius);
...
IC	f32 CCoverEvaluatorBase::best_value				() const
{
	return					(m_best_value);
}
...
IC	void CCoverEvaluatorCloseToEnemy::setup(const fVector3& enemy_position, f32 min_enemy_distance, f32 max_enemy_distance, f32 deviation)
{
	inherited::setup		();
...
IC	void CCoverEvaluatorAngle::setup(const fVector3& enemy_position, f32 min_enemy_distance, f32 max_enemy_distance, u32 level_vertex_id)
{
...
IC	void CCoverEvaluatorSafe::setup		(f32 min_distance)
{
	inherited::setup		();
