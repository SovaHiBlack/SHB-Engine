...
#	if 0
							...
							if (weapon_object) {
								...
								if (weapon) {
									...
									f32 dist_factor	= _min(1.f,fly_dist/Level().BulletManager().m_fHPMaxDist);
...
void CBulletManager::FireShotmark (SBullet* bullet, const Fvector& vDir, const Fvector &vEnd, collide::rq_result& R, u16 target_material, const Fvector& vNormal, bool ShowMark)
{
	...
	pcstr ps_name = (!mtl_pair || mtl_pair->CollideParticles.empty())?
...
std::pair<f32, f32>  CBulletManager::ObjectHit	(SBullet* bullet, const Fvector& end_point,
...
//	if(shoot_factor<RICOCHET_THRESHOLD &&  )
	if (((f+shoot_factor)<ricoshet_factor) && bullet->flags.allow_ricochet)	{
		...
		//f32 speed_lost = fis_zero(bullet->hit_impulse) ?	1.f : 		1.f - impulse/bullet->hit_impulse;
		//clamp (speed_lost, 0.f , 1.f);
		//f32 speed_lost = shoot_factor;
