...
public:
	...
	static	void				rot_dx					(f32 val){Fvector v = get_angle_offset(); v.x +=val;set_angle_offset(v);}
	static	void				rot_dy					(f32 val){Fvector v = get_angle_offset(); v.y +=val;set_angle_offset(v);}
	static	void				rot_dz					(f32 val){Fvector v = get_angle_offset(); v.z +=val;set_angle_offset(v);}

	static	void				mov_dx					(f32 val){Fvector c = get_pos_offset(); c.x +=val; m_dbgItem->m_offset.c=c;}
	static	void				mov_dy					(f32 val){Fvector c = get_pos_offset(); c.y +=val; m_dbgItem->m_offset.c=c;}
	static	void				mov_dz					(f32 val){Fvector c = get_pos_offset(); c.z +=val; m_dbgItem->m_offset.c=c;}
#endif
};
