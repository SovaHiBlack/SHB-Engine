...
// what to block
unsigned int INV_STATE_LADDER = (1 << RIFLE_SLOT);
unsigned int INV_STATE_CAR = INV_STATE_LADDER;
unsigned int INV_STATE_BLOCK_ALL = 0xffffffff;
unsigned int INV_STATE_INV_WND = INV_STATE_BLOCK_ALL;
unsigned int INV_STATE_BUY_MENU = INV_STATE_BLOCK_ALL;
...
CInventory::CInventory( )
{
	...
	m_iLoadActiveSlotFrame = unsigned int(-1);
}
...
void CInventory::Activate_deffered(unsigned int slot, unsigned int _frame)
{
...
bool CInventory::Activate(unsigned int slot, EActivationReason reason, bool bForce)
{
	...
	if (m_iActiveSlot == NO_ACTIVE_SLOT)	//активный слот не выбран
	{
	...
	else if (slot == NO_ACTIVE_SLOT || m_slots[slot].m_pIItem)	//активный слот задействован
	{
...
PIItem CInventory::ItemFromSlot(unsigned int slot) const
{
	VERIFY(NO_ACTIVE_SLOT != slot);
	return m_slots[slot].m_pIItem;
}

void CInventory::SendActionEvent(int cmd, unsigned int flags)
{
...
bool CInventory::Action(int cmd, unsigned int flags)
{
	CActor* pActor = smart_cast<CActor*>(m_pOwner);
	if (pActor)
	{
...
void CInventory::UpdateDropItem(PIItem pIItem)
{
	if (pIItem->GetDropManual( ))
	{
		...
		if (OnServer( ))
		{
			...
			P.w_u16(unsigned short(pIItem->object( ).ID( )));
...
PIItem CInventory::SameSlot(const unsigned int slot, PIItem pIItem, bool bSearchRuck) const
{
...
PIItem CInventory::Get(const unsigned short id, bool bSearchRuck) const
{
	const TIItemContainer& list = bSearchRuck ? m_ruck : m_belt;
...
unsigned int CInventory::dwfGetSameItemCount(const char* caSection, bool SearchAll)
{
	unsigned int l_dwCount = 0;
...
unsigned int CInventory::dwfGetGrenadeCount(const char* caSection, bool SearchAll)
{
	unsigned int l_dwCount = 0;
	TIItemContainer& l_list = SearchAll ? m_all : m_ruck;
	for (TIItemContainer::iterator l_it = l_list.begin( ); l_list.end( ) != l_it; ++l_it)
...
bool CInventory::CanPutInSlot(PIItem pIItem) const
{
	...
	if (pIItem->GetSlot( ) < m_slots.size( ) && m_slots[pIItem->GetSlot( )].m_pIItem == nullptr)
	{
		return true;
	}
...
unsigned int CInventory::dwfGetObjectCount( )
{
	return m_all.size( );
}
...
CInventoryItem* CInventory::GetItemFromInventory(const char* caItemName)
{
	...
	unsigned int crc = crc32(caItemName, xr_strlen(caItemName));
...
unsigned int CInventory::BeltWidth( ) const
{
	return m_iMaxBelt;
}
...
void CInventory::AddAvailableItems(TIItemContainer& items_container, bool for_trade) const
{
	...
	if (m_bSlotsUseful)
	{
		InventorySlotVec::const_iterator slot_it = m_slots.begin( );
		InventorySlotVec::const_iterator slot_it_e = m_slots.end( );
...
{
	InventorySlotVec::iterator it = m_slots.begin( );
...
void CInventory::SetSlotsBlocked(unsigned short mask, bool bBlock)
{
	bool bChanged = false;
	...
	if (bChanged)
	{
		unsigned int ActiveSlot = GetActiveSlot( );
		unsigned int PrevActiveSlot = GetPrevActiveSlot( );
