...
float CEntityAlive::m_fBloodMarkSizeMin = 0.0f;
float CEntityAlive::m_fBloodMarkSizeMax = 0.0f;
float CEntityAlive::m_fBloodMarkDistance = 0.0f;
float CEntityAlive::m_fNominalHit = 0.0f;
...
///////////////////////////////////////////////////////////////////////
unsigned short CEntityAlive::PHGetSyncItemsNumber( )
{
...
CPHSynchronize* CEntityAlive::PHGetSyncItem(unsigned short item)
{
	if (character_physics_support( )->movement( )->CharacterExist( ))
	{
...
{
	if (BI_NONE == (unsigned short) element)
	{
		return;
	}
	...
	if (V)
	{
		Fmatrix& m_bone = (V->LL_GetBoneInstance(unsigned short(element))).mTransform;
...
void CEntityAlive::PlaceBloodWallmark(const Fvector3& dir, const Fvector3& start_pos, float trace_dist, float wallmark_size, SHADER_VECTOR& wallmarks_vector)
{
...
void CEntityAlive::StartFireParticles(CWound* pWound)
{
	if (pWound->TypeSize(ALife::eHitTypeBurn) > m_fStartBurnWoundSize)
	{
		...
		unsigned short particle_bone = CParticlesPlayer::GetNearestBone(V, pWound->GetBoneNum( ));
...
{
	int relation = CMonsterCommunity::relation(this->monster_community->index( ), tpEntityAlive->monster_community->index( ));
	...
	for (WOUND_VECTOR_IT it = m_BloodWounds.begin( ); it != m_BloodWounds.end( );)
	{
		...
		if (pWound->m_fDropTime < Device.fTimeGlobal)
		{
			...
			if (pWound->GetBoneNum( ) != BI_NONE)
			{
				...
				CParticlesPlayer::GetBonePos(this, pWound->GetBoneNum( ), Fvector3( ).set(0, 0, 0), pos);
				pos.add(pos_distort);
				PlaceBloodWallmark(Fvector3( ).set(0.f, -1.f, 0.f), pos, m_fBloodMarkDistance, m_fBloodDropSize, *m_pBloodDropsVector);
			}
		}
