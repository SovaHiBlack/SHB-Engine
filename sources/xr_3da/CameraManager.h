...
struct ENGINE_API SPPInfo
{
	struct SColor
	{
		...
		IC operator u32( )
		{
			s32		_r = clampr(iFloor(r * 255.0f + 0.5f), 0, 255);
			s32		_g = clampr(iFloor(g * 255.0f + 0.5f), 0, 255);
			s32		_b = clampr(iFloor(b * 255.0f + 0.5f), 0, 255);
...
public:
	...
	IC fVector3				Dir( ) const
	{
		return vDirection;
	}
	IC fVector3				Up( ) const
	{
		return vNormal;
	}
	IC fVector3				Right( ) const
	{
		return vRight;
	}

	IC void					camera_Matrix(fMatrix4x4& M)
	{
