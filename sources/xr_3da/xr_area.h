...
private:
	BOOL								_RayTest			( const Fvector &start, const Fvector &dir, f32 range, collide::rq_target tgt, collide::ray_cache* cache, CObject* ignore_object);
	BOOL								_RayPick			( const Fvector &start, const Fvector &dir, f32 range, collide::rq_target tgt, collide::rq_result& R, CObject* ignore_object );
...
public:
	...
	// Occluded/No
	BOOL								RayTest				( const Fvector &start, const Fvector &dir, f32 range, collide::rq_target tgt, collide::ray_cache* cache, CObject* ignore_object);

	// Game raypick (nearest) - returns object and addititional params
	BOOL								RayPick				( const Fvector &start, const Fvector &dir, f32 range, collide::rq_target tgt, collide::rq_result& R, CObject* ignore_object );
