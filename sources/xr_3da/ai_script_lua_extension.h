...
namespace Script {
	...
	s32					LuaPanic					(CLuaVirtualMachine *tpLuaVM);
#endif
	bool				bfPrintOutput				(CLuaVirtualMachine *tpLuaVM, pcstr	caScriptName, s32 iErorCode = 0);
	pcstr				cafEventToString			(s32				iEventCode);
	void				vfPrintError				(CLuaVirtualMachine *tpLuaVM, s32		iErrorCode);
	bool				bfListLevelVars				(CLuaVirtualMachine *tpLuaVM, s32		iStackLevel);
	...
	bool				bfIsObjectPresent			(CLuaVirtualMachine *tpLuaVM, pcstr	caIdentifier, s32 type);
	bool				bfIsObjectPresent			(CLuaVirtualMachine *tpLuaVM, pcstr	caNamespaceName, pcstr	caIdentifier, s32 type);
