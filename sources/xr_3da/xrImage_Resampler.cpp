...
struct Image
{
	s32		xsize;		/* horizontal size of the image in Pixels */
	s32		ysize;		/* vertical size of the image in Pixels */
	Pixel* data;		/* pointer to first scanline of image */
	s32		span;		/* byte offset between two scanlines */
};
...
Image* new_image(s32 xsize, s32 ysize)		/* create a blank image */
{
	Image* image;
...
Pixel	get_pixel(Image* image, s32 x, s32 y)
{
...
void	get_row(Pixel* row, Image* image, s32 y)
{
...
void	get_column(Pixel* column, Image* image, s32 x)
{
	s32 i;
	s32 d;
	Pixel* p;
...
Pixel	put_pixel(Image* image, s32 x, s32 y, Pixel data)
{
	if ((x < 0) || (x >= image->xsize) || (y < 0) || (y >= image->ysize))
	{
		return 0;
