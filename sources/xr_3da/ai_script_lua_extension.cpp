...
void Script::vfLoadStandardScripts(CLuaVirtualMachine *tpLuaVM)
{
	...
	pcstr			caScriptString = l_tpIniFile->r_string("common","script");
...
bool Script::bfLoadFile(CLuaVirtualMachine *tpLuaVM, pcstr caScriptName, bool bCall)
{
...
bool bfCreateNamespaceTable(CLuaVirtualMachine *tpLuaVM, pcstr caNamespaceName)
{
...
bool bfDoFile(CLuaVirtualMachine *tpLuaVM, pcstr caScriptName, pcstr caNameSpaceName, bool bCall)
{
	...
	if (!bfLoadBuffer(tpLuaVM,static_cast<pcstr>(l_tpFileReader->pointer()),(size_t)l_tpFileReader->length(),l_caLuaFileName,caNameSpaceName)) {
		lua_pop			(tpLuaVM,4);
...
bool Script::bfLoadFileIntoNamespace(CLuaVirtualMachine *tpLuaVM, pcstr caScriptName, pcstr caNamespaceName, bool bCall)
{
...
bool Script::bfGetNamespaceTable(CLuaVirtualMachine *tpLuaVM, pcstr N)
{
	lua_pushstring 		(tpLuaVM,"_G"); 
	lua_gettable 		(tpLuaVM,LUA_GLOBALSINDEX); 
...
CLuaVirtualMachine *Script::get_namespace_table(CLuaVirtualMachine *tpLuaVM, pcstr N)
{
	if (!xr_strlen(N))
		return				(tpLuaVM);
	lua_pushstring 			(tpLuaVM,"_G"); 
...
luabind::object Script::lua_namespace_table(CLuaVirtualMachine *tpLuaVM, pcstr namespace_name)
{
	string256			S1;
