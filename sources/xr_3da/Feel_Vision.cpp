...
namespace Feel
{
	...
	struct SFeelParam
	{
		...
		f32 vis;
		f32 vis_threshold;
		SFeelParam(Vision* _parent, Vision::feel_visible_Item* _item, f32 _vis_threshold) : parent(_parent), item(_item), vis(1.0f), vis_threshold(_vis_threshold)
		{ }
	};
	IC BOOL feel_vision_callback(collide::rq_result& result, pvoid params)
	{
	...
	void	Vision::o_new(CObject* O)
	{
		...
		I.O = O;
		I.Cache_vis = 1.0f;
		I.Cache.verts[0].set(0.0f, 0.0f, 0.0f);
		I.Cache.verts[1].set(0.0f, 0.0f, 0.0f);
		I.Cache.verts[2].set(0.0f, 0.0f, 0.0f);
		I.fuzzy = -EPSILON_7;
		I.cp_LP.set(0.0f, 0.0f, 0.0f);
	}
