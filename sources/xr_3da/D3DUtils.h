...
public:
    ...
    //----------------------------------------------------
    virtual void __stdcall DrawCross(const Fvector& p, F32 szx1, F32 szy1, F32 szz1, F32 szx2, F32 szy2, F32 szz2, u32 clr, BOOL bRot45=false);
    virtual void __stdcall DrawCross(const Fvector& p, F32 sz, u32 clr, BOOL bRot45=false){ DrawCross(p, sz,sz,sz, sz,sz,sz, clr, bRot45); }
    virtual void __stdcall DrawEntity(u32 clr, ref_shader s);
    virtual void __stdcall DrawFlag(const Fvector& p, F32 heading, F32 height, F32 sz, F32 sz_fl, u32 clr, BOOL bDrawEntity);
    virtual void __stdcall DrawRomboid(const Fvector& p, F32 radius, u32 clr);
	virtual void __stdcall DrawJoint(const Fvector& p, F32 radius, u32 clr);

    virtual void __stdcall DrawSpotLight(const Fvector& p, const Fvector& d, F32 range, F32 phi, u32 clr);
    virtual void __stdcall DrawDirectionalLight(const Fvector& p, const Fvector& d, F32 radius, F32 range, u32 clr);
    virtual void __stdcall DrawPointLight(const Fvector& p, F32 radius, u32 clr);
    ...
    virtual void __stdcall DrawLink(const Fvector& p0, const Fvector& p1, F32 sz, u32 clr);
    IC virtual void __stdcall DrawFaceNormal(const Fvector& p0, const Fvector& p1, const Fvector& p2, F32 size, u32 clr){
        Fvector N,C,P; N.mknormal(p0,p1,p2); C.set(p0);C.add(p1);C.add(p2);C.div(3);
        P.mad(C,N,size);
        DrawLine(C,P,clr);}
    IC virtual void __stdcall DrawFaceNormal(const Fvector* p, F32 size, u32 clr){DrawFaceNormal(p[0],p[1],p[2],size,clr);}
	...
	virtual void __stdcall DrawSphere	(const Fmatrix& parent, const Fvector& center, F32 radius, u32 clr_s, u32 clr_w, BOOL bSolid, BOOL bWire);
	...
	virtual void __stdcall DrawCylinder(const Fmatrix& parent, const Fvector& center, const Fvector& dir, F32 height, F32 radius, u32 clr_s, u32 clr_w, BOOL bSolid, BOOL bWire);
	virtual void __stdcall DrawCone	(const Fmatrix& parent, const Fvector& apex, const Fvector& dir, F32 height, F32 radius, u32 clr_s, u32 clr_w, BOOL bSolid, BOOL bWire);
    ...
    virtual void __stdcall DrawPivot	(const Fvector& pos, F32 sz=5.0f);
	virtual void __stdcall DrawAxis	(const Fmatrix& T);
	virtual void __stdcall DrawObjectAxis(const Fmatrix& T, F32 sz, BOOL sel);
