...
void CEnvDescriptorMixer::lerp(CEnvironment*, CEnvDescriptor& A, CEnvDescriptor& B, f32 f, CEnvModifier& M, f32 m_power)
{
	...
	VERIFY2(						sun_dir.y < 0.0f, "Invalid sun direction settings while lerp");
}
...
void CEnvironment::load( )
{
	...
	if (!eff_Rain)
	{
		eff_Rain = xr_new<CRainEffect>( );
	}

	if (!eff_LensFlare)
	{
		eff_LensFlare = xr_new<CLensFlareEffect>( );
	}

	if (!eff_Thunderbolt)
	{
		eff_Thunderbolt = xr_new<CThunderboltEffect>( );
	}
