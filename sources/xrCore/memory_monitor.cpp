...
void memory_monitor::monitor_alloc(pcvoid allocation_address, const u32& allocation_size, pcstr allocation_description)
{
	if (!use_monitor())
		return;
...
void memory_monitor::monitor_free(pcvoid deallocation_address)
{
	if (!use_monitor())
		return;
