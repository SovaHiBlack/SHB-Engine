...
//.		IC void			clear()		{ Data.clear();			}
    ...
    static IC BOOL		IsBOOL		(pcstr B)	{ return (xr_strcmp(B,"on")==0 || xr_strcmp(B,"yes")==0 || xr_strcmp(B,"true")==0 || xr_strcmp(B,"1")==0);}
private:
	LPSTR		fName;
	Root		DATA;
	BOOL		bReadOnly;
...
public:
	...
	CLASS_ID	r_clsid			( const shared_str& S, pcstr L )				{ return r_clsid(*S,L);			}
	pcstr 		r_string		(pcstr S, pcstr L);															// оставляет кавычки
	pcstr 		r_string		( const shared_str& S, pcstr L)				{ return r_string(*S,L);		}	// оставляет кавычки
	shared_str		r_string_wb		(pcstr S, pcstr L);															// убирает кавычки
	shared_str		r_string_wb		( const shared_str& S, pcstr L)				{ return r_string_wb(*S,L);		}	// убирает кавычки
	u8	 		r_u8			(pcstr S, pcstr L );
	u8	 		r_u8			( const shared_str& S, pcstr L )				{ return r_u8(*S,L);			}
	u16	 		r_u16			(pcstr S, pcstr L );
	u16	 		r_u16			( const shared_str& S, pcstr L )				{ return r_u16(*S,L);			}
	u32	 		r_u32			(pcstr S, pcstr L );
	u32	 		r_u32			( const shared_str& S, pcstr L )				{ return r_u32(*S,L);			}
	s8	 		r_s8			(pcstr S, pcstr L );
	s8	 		r_s8			( const shared_str& S, pcstr L )				{ return r_s8(*S,L);			}
	s16	 		r_s16			(pcstr S, pcstr L );
	s16	 		r_s16			( const shared_str& S, pcstr L )				{ return r_s16(*S,L);			}
	s32	 		r_s32			(pcstr S, pcstr L );
	s32	 		r_s32			( const shared_str& S, pcstr L )				{ return r_s32(*S,L);			}
	F32			r_float			(pcstr S, pcstr L );
	F32			r_float			( const shared_str& S, pcstr L )				{ return r_float(*S,L);			}
	Fcolor		r_fcolor		(pcstr S, pcstr L );
	Fcolor		r_fcolor		( const shared_str& S, pcstr L )				{ return r_fcolor(*S,L);		}
	u32			r_color			(pcstr S, pcstr L );
	u32			r_color			( const shared_str& S, pcstr L )				{ return r_color(*S,L);			}
	Ivector2	r_ivector2		(pcstr S, pcstr L );
	Ivector2	r_ivector2		( const shared_str& S, pcstr L )				{ return r_ivector2(*S,L);		}
	Ivector3	r_ivector3		(pcstr S, pcstr L );
	Ivector3	r_ivector3		( const shared_str& S, pcstr L )				{ return r_ivector3(*S,L);		}
	Ivector4	r_ivector4		(pcstr S, pcstr L );
	Ivector4	r_ivector4		( const shared_str& S, pcstr L )				{ return r_ivector4(*S,L);		}
	Fvector2	r_fvector2		(pcstr S, pcstr L );
	Fvector2	r_fvector2		( const shared_str& S, pcstr L )				{ return r_fvector2(*S,L);		}
	Fvector3	r_fvector3		(pcstr S, pcstr L );
	Fvector3	r_fvector3		( const shared_str& S, pcstr L )				{ return r_fvector3(*S,L);		}
	Fvector4	r_fvector4		(pcstr S, pcstr L );
	Fvector4	r_fvector4		( const shared_str& S, pcstr L )				{ return r_fvector4(*S,L);		}
	BOOL		r_bool			(pcstr S, pcstr L );
	BOOL		r_bool			( const shared_str& S, pcstr L )				{ return r_bool(*S,L);			}
