...
public:
	...
	{
		x = _abs(p.x); y = _abs(p.y);		return *this;
	}
	IC SelfRef min(const Self& p)
	{
		x = _min(x, p.x); y = _min(y, p.y);	return *this;
	}
	IC SelfRef min(T _x, T _y)
	{
		x = _min(x, _x);  y = _min(y, _y);	return *this;
	}
	IC SelfRef max(const Self& p)
	{
		x = _max(x, p.x); y = _max(y, p.y);	return *this;
	}
	IC SelfRef max(T _x, T _y)
	{
		x = _max(x, _x);  y = _max(y, _y);	return *this;
	}
	IC SelfRef sub(T p)
	{
		x -= p; y -= p;					return *this;
	...
	{
		F32 t = -x; x = y; y = t;			return *this;
	}
	IC SelfRef cross(Self& D)
	{
		x = D.y; y = -D.x;            return *this;
	}
	IC T dot(Self& p)
	{
		return x * p.x + y * p.y;
	...
	{
		F32 m = _sqrt(x * x + y * y); x /= m; y /= m;			return *this;
	}
	IC SelfRef norm_safe(void)
	{
		F32 m = _sqrt(x * x + y * y); if (m)
		{
			x /= m; y /= m;
		}	return *this;
	}
	...
	{
		F32 m = _sqrt(v.x * v.x + v.y * v.y); x = v.x / m; y = v.y / m;			return *this;
	}
	IC SelfRef 	normalize_safe(const Self& v)
	{
		F32 m = _sqrt(v.x * v.x + v.y * v.y); if (m)
		{
			x = v.x / m; y = v.y / m;
		}	return *this;
	}
