...
public:
	...
	{
		min.set(x1, y1);	max.set(x2, y2);					return *this;
	};
	...
	IC	SelfRef	null()
	{
		min.set(0.f, 0.f);	max.set(0.f, 0.f);				return *this;
	};
	IC	SelfRef	identity()
	{
	...
	{
		min.set(type_max(T), type_max(T)); max.set(type_min(T), type_min(T)); return *this;
	}

	IC	SelfRef	shrink(T s)
	{
		min.add(s); max.sub(s);				return *this;
	};
	...
	IC	SelfRef	grow(T s)
	{
		min.sub(s); max.add(s);				return *this;
	};
	...
	{
		min.add(p); max.add(p);				return *this;
	};
	...
	{
		Tvector R; getsize(R); R.mul(0.5f);	return R.magnitude();
	};
