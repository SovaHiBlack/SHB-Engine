...
void	CHW::selectResolution(unsigned int& dwWidth, unsigned int& dwHeight, BOOL bWindowed)
{
	...
	else //check
	{
		...
		if (_ParseItem(buff, vid_mode_token) == unsigned int(-1)) //not found
		{ //select safe
...
//. #endif // DEBUG
	...
	Msg("* GPU driver: %d.%d.%d.%d", unsigned int(drv_Product), unsigned int(drv_Version), unsigned int(drv_SubVersion), unsigned int(drv_Build));
...
// Fatal error! Cannot create rendering device AT STARTUP !!!
	...
	R_CHK(pDevice->GetRenderTarget(0, &pBaseRT));
	R_CHK(pDevice->GetDepthStencilSurface(&pBaseZB));
	unsigned int	memory = pDevice->GetAvailableTextureMem( );
...
unsigned int	CHW::selectPresentInterval( )
{
	D3DCAPS9	caps;
...
unsigned int CHW::selectRefresh(unsigned int dwWidth, unsigned int dwHeight, D3DFORMAT fmt)
{
	if (psDeviceFlags.is(rsRefresh60hz))	return D3DPRESENT_RATE_DEFAULT;
	else
	{
		unsigned int selected = D3DPRESENT_RATE_DEFAULT;
		unsigned int count = pD3D->GetAdapterModeCount(DevAdapter, fmt);
...
void	fill_vid_mode_list(CHW* _hw)
{
	...
	unsigned int cnt = _hw->pD3D->GetAdapterModeCount(_hw->DevAdapter, _hw->Caps.fTarget);
