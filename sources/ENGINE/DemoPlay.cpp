...
CDemoPlay::CDemoPlay(const char* name, float ms, unsigned int cycles, float life_time) : CEffectorCam(cetDemo, life_time)
{
...
void CDemoPlay::stat_Stop( )
{
	...
	// total
	unsigned int dwFramesTotal = Device.dwFrame - stat_StartFrame;
	...
	rfps_max = flt_min;
	rfps_middlepoint = 0;
	for (unsigned int it = 1; it < stat_table.size( ); it++)
	{
