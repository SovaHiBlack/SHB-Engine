...
//---------------------------------------------------------------------
ENGINE_API CConfigurationFile* pGameIni	= nullptr;
...
static int start_year	= 2021;	// 2021
...
struct _SoundProcessor : public pureFrame
{
	...
	{
//		Msg("------------- sound: %d [%3.2f,%3.2f,%3.2f]",unsigned int(Device.dwFrame),VPUSH(Device.vCameraPosition));
...
void InitSettings( )
{
	...
	pSettings = xr_new<CConfigurationFile>(fname, TRUE);
	CHECK_OR_EXIT(!pSettings->sections( ).empty( ), make_string("Cannot find file %s.\nReinstalling application may fix this problem.", fname));

	FS.update_path(fname, "$game_config$", "game.ltx");
	pGameIni = xr_new<CConfigurationFile>(fname, TRUE);
...
#else
	unsigned int h = Device.dwHeight;
...
void _InitializeFont(CGameFont*& F, const char* section, unsigned int flags)
{
...
unsigned int calc_progress_color(unsigned int, unsigned int, int, int);
...
void CApplication::Level_Set(unsigned int ID)
{
	if (ID >= Levels.size( ))
	{
		return;
...
void doBenchmark(const char* name)
{
	...
	CConfigurationFile ini(in_file);
...
void CApplication::load_draw_internal( )
{
	...
	for (unsigned int idx = 0; idx < v_cnt + 1; ++idx)
	{
	...
	VERIFY(unsigned int(pv - _pv) == 2 * (v_cnt + 1));
...
unsigned int calc_progress_color(unsigned int idx, unsigned int total, int stage, int max_stage)
{
	if (idx > (total / 2))
	{
		idx = total - idx;
