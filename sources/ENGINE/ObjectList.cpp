...
public:
	...
	{ };
	inline bool operator() (CObject* O)
	{
		return cls == O->CLS_ID;
	}
};
...
{
	for (unsigned int _it = 0; _it < o.size( ); _it++)	o[_it]->IAmNotACrowAnyMore( );
	o.clear_not_free( );
}
...
void CObjectList::Update(bool bForce)
{
	if (!(Device.Paused( ) && !bForce))
	{
		...
		if (Device.fTimeDelta > EPS_S || bForce)
		{
			...
			unsigned int objects_count = workload->size( );
...
{
	xr_map<unsigned int, CObject*>::iterator	it = map_NETID.find(O->ID( ));
...
unsigned int	CObjectList::net_Export(CNetPacket* _Packet, unsigned int start, unsigned int max_object_size)
{
	...
	for (; start < objects_active.size( ) + objects_sleeping.size( ); start++)
	{
		...
		{
			Packet.w_u16(unsigned short(P->ID( )));
			Packet.w_chunk_open8(position);
			//Msg						("cl_export: %d '%s'",P->ID(),*P->cName());
			P->net_Export(Packet);

#ifdef DEBUG
			unsigned int size = unsigned int(Packet.w_tell( ) - position) - sizeof(unsigned char);
			if (size >= 256)
			{
...
void CObjectList::net_Import(CNetPacket* Packet)
{
	...
	while (!Packet->r_eof( ))
	{
		...
		unsigned char size;
		Packet->r_u8(size);
		CObject* P = net_Find(unsigned int(ID));
		if (P)
		{

			unsigned int rsize = Packet->r_tell( );
...
CObject* CObjectList::net_Find(unsigned int ID)
{
	xr_map<unsigned int, CObject*>::iterator	it = map_NETID.find(ID);
