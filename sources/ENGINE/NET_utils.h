...
public:
	...
	// writing - utilities
	inline void	w_float		( float a       )	{ w(&a,4);					}			// float
	inline void w_vec3		( const Fvector3& a) { w(&a,3*sizeof(float));	}			// vec3
	inline void w_vec4		( const Fvector4& a){ w(&a,4*sizeof(float));	}			// vec4
	inline void w_u64		( u64 a			)	{ w(&a,8);					}			// qword (8b)
	inline void w_s64		( s64 a			)	{ w(&a,8);					}			// qword (8b)
	inline void w_u32		( u32 a			)	{ w(&a,4);					}			// dword (4b)
	inline void w_s32		(int a			)	{ w(&a,4);					}			// dword (4b)
	inline void w_u24		( u32 a			)	{ w(&a,3);					}			// dword (3b)
	inline void w_u16		( u16 a			)	{ w(&a,2);					}			// word (2b)
	inline void w_s16		( s16 a			)	{ w(&a,2);					}			// word (2b)
	inline void	w_u8		( u8 a			)	{ w(&a,1);					}			// byte (1b)
	inline void	w_s8		( s8 a			)	{ w(&a,1);					}			// byte (1b)
	...
	inline u32		r_tell			()	{ return r_pos; }
	...
	// reading - utilities
	inline void		r_vec3			(Fvector3& A)	{ r(&A,3*sizeof(float));		} // vec3
	inline void		r_vec4			(Fvector4& A)	{ r(&A,4*sizeof(float));		} // vec4
	inline void		r_float			(float& A )		{ r(&A,4);						} // float
	inline void 	r_u64			(u64& A)		{ r(&A,8);						} // qword (8b)
	inline void 	r_s64			(s64& A)		{ r(&A,8);						} // qword (8b)
	inline void 	r_u32			(u32& A)		{ r(&A,4);						} // dword (4b)
	inline void		r_s32			(int& A)		{ r(&A,4);						} // dword (4b)
	inline void		r_u24			(u32& A)		{ A=0; r(&A,3);					} // dword (3b)
	inline void		r_u16			(u16& A)		{ r(&A,2);						} // word (2b)
	inline void		r_s16			(s16& A)		{ r(&A,2);						} // word (2b)
	inline void		r_u8			(u8&  A)		{ r(&A,1);						} // byte (1b)
	inline void		r_s8			(s8&  A)		{ r(&A,1);						} // byte (1b)
	// IReader compatibility
	inline Fvector3	r_vec3			()		{
		Fvector3 A;r(&A,3*sizeof(float));	return(A);		} // vec3
	inline Fvector4	r_vec4			()		{Fvector4 A;r(&A,4*sizeof(float));	return(A);		} // vec4
	inline float	r_float_q8		(float min,float max){float A;r_float_q8(A,min,max);return A;}
	inline float	r_float_q16		(float min, float max){float A;r_float_q16(A,min,max);return A;}
	inline float	r_float			()		{ float A; r(&A,4);					return(A);		} // float
	inline u64 		r_u64			()		{ u64 	A; r(&A,8);					return(A);		} // qword (8b)
	inline s64 		r_s64			()		{ s64 	A; r(&A,8);					return(A);		} // qword (8b)
	inline u32 		r_u32			()		{ u32 	A; r(&A,4);					return(A);		} // dword (4b)
	inline int		r_s32			()		{ int	A; r(&A,4);					return(A);		} // dword (4b)
	inline u32		r_u24			()		{ u32	A=0; r(&A,3);				return(A);		} // dword (3b)
	inline u16		r_u16			()		{ u16	A; r(&A,2);					return(A);		} // word (2b)
	inline s16		r_s16			()		{ s16	A; r(&A,2);					return(A);		} // word (2b)
	inline u8		r_u8			()		{ u8	A; r(&A,1);					return(A);		} // byte (1b)
	inline s8		r_s8			()		{ s8	A; r(&A,1);					return(A);		} // byte (1b)
	...
	inline void		r_angle16		(float& A)		{ r_float_q16	(A,0,PI_MUL_2);	}
	inline void		r_angle8		(float& A)		{ r_float_q8	(A,0,PI_MUL_2);	}
	inline void		r_dir			(Fvector3& A)	{ u16 t; r_u16(t); pvDecompress  (A,t); }
