...
public:
	adopt_sampler(CBlender_Compile* _C, unsigned int _stage) : C(_C), stage(_stage)
	{
		if (unsigned int(-1) == stage) C = 0;
	}
...
public:
	...
	adopt_compiler& _blend(bool	_blend, unsigned int abSRC, unsigned int abDST)
	{
		C->PassSET_ablend_mode(_blend, abSRC, abDST);	return 	*this;
	}
	adopt_compiler& _aref(bool	_aref, unsigned int aref)
	{
		C->PassSET_ablend_aref(_aref, aref);			return 	*this;
	}
	adopt_sampler			_sampler(const char* _name)
	{
		unsigned int s = C->r_Sampler(_name, 0);				return	adopt_sampler(C, s);
	}
};
...
ENGINE_API unsigned int engine_lua_memory_usage( )
{
	return			((unsigned int) dlmallinfo( ).uordblks);
}
...
void	CResourceManager::LS_Load( )
{
	...
	VERIFY(folder);
	for (unsigned int it = 0; it < folder->size( ); it++)
	{
