...
void SPostProcessInfo::normalize( )
{
	/*
...
void SPostProcessInfo::validate(const char* str)
{
...
{
	for (EffectorCamVec_it it = m_EffectorsCam.begin( ); it != m_EffectorsCam.end( ); it++)
	{
		xr_delete(*it);
	}

	for (EffectorPostProcessVec_it it = m_EffectorsPP.begin( ); it != m_EffectorsPP.end( ); it++)
	{
		xr_delete(*it);
	}
}
...
{
	EffectorCamVec_it it = m_EffectorsCam_added_deffered.begin( );
	EffectorCamVec_it it_e = m_EffectorsCam_added_deffered.end( );
	for (; it != it_e; ++it)
	{
...
CEffectorPostProcess* CCameraManager::GetPPEffector(EEffectorPostProcessType type)
{
	for (EffectorPostProcessVec_it it = m_EffectorsPP.begin( ); it != m_EffectorsPP.end( ); it++)
	{
		if ((*it)->Type( ) == type)
...
CEffectorPostProcess* CCameraManager::AddPPEffector(CEffectorPostProcess* ef)
{
...
void CCameraManager::Update(const Fvector3& P, const Fvector3& D, const Fvector3& N, float fFOV_Dest, float fASPECT_Dest, float fFAR_Dest, unsigned int flags)
{
	...
	if (m_EffectorsPP.size( ))
	{
		...
		{
			CEffectorPostProcess* eff = m_EffectorsPP[i];
			SPostProcessInfo l_PPInf = pp_zero;
