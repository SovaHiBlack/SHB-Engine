...
static unsigned short			hbox_faces[20 * 3] =
{
	0,	 2,	 3,
	3,	 1,	 0,
	4,	 5,	 7,
...
struct v_skybox
{
	...
	void		set(Fvector3& _p, unsigned int _c, Fvector3& _tc)
	{
		p = _p;
		color = _c;
		uv[0] = _tc;
...
const	unsigned int v_skybox_fvf = D3DFVF_XYZ | D3DFVF_DIFFUSE | D3DFVF_TEX2 | D3DFVF_TEXCOORDSIZE3(0) | D3DFVF_TEXCOORDSIZE3(1);
struct v_clouds
{
	Fvector3	p;
	unsigned int			color;
	unsigned int			intensity;
	void		set(Fvector3& _p, unsigned int _c, unsigned int _i)
	{
		p = _p;
		color = _c;
		intensity = _i;
...
const	unsigned int v_clouds_fvf = D3DFVF_XYZ | D3DFVF_DIFFUSE | D3DFVF_SPECULAR;
#pragma pack(pop)
...
void CEnvironment::RenderSky( )
{
	...
	unsigned int		C = color_rgba(iFloor(CurrentEnv.sky_color.x * 255.f), iFloor(CurrentEnv.sky_color.y * 255.f), iFloor(CurrentEnv.sky_color.z * 255.f), iFloor(CurrentEnv.weight * 255.f));

	// Fill index buffer
	unsigned short* pib = RCache.Index.Lock(20 * 3, i_offset);
...
void CEnvironment::RenderClouds( )
{
	...
	unsigned int		i_offset;
	unsigned int		v_offset;
	unsigned int		C0 = color_rgba(iFloor(wind_dir.x), iFloor(wind_dir.y), iFloor(wind_dir.w), iFloor(wind_dir.z));
	unsigned int		C1 = color_rgba(iFloor(CurrentEnv.clouds_color.x * 255.0f), iFloor(CurrentEnv.clouds_color.y * 255.0f), iFloor(CurrentEnv.clouds_color.z * 255.0f), iFloor(CurrentEnv.clouds_color.w * 255.0f));

	// Fill index buffer
	unsigned short* pib = RCache.Index.Lock(CloudsIndices.size( ), i_offset);
	CopyMemory(pib, &CloudsIndices.front( ), CloudsIndices.size( ) * sizeof(unsigned short));
