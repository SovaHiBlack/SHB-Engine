...
void	CBlender_Compile::PassSET_ablend_mode(BOOL bABlend, unsigned int abSRC, unsigned int abDST)
{
...
void	CBlender_Compile::PassSET_ablend_aref(BOOL bATest, unsigned int aRef)
{
	clamp(aRef, 0u, 255u);
	RS.SetRS(D3DRS_ALPHATESTENABLE, BC(bATest));
	if (bATest)	RS.SetRS(D3DRS_ALPHAREF, unsigned int(aRef));
}

void	CBlender_Compile::PassSET_Blend(BOOL bABlend, unsigned int abSRC, unsigned int abDST, BOOL bATest, unsigned int aRef)
{
...
void	CBlender_Compile::StageSET_Address(unsigned int adr)
{
	RS.SetSAMP(Stage( ), D3DSAMP_ADDRESSU, adr);
	RS.SetSAMP(Stage( ), D3DSAMP_ADDRESSV, adr);
}
void	CBlender_Compile::StageSET_XForm(unsigned int tf, unsigned int tc)
{ }
void	CBlender_Compile::StageSET_Color(unsigned int a1, unsigned int op, unsigned int a2)
{
	RS.SetColor(Stage( ), a1, op, a2);
}
void	CBlender_Compile::StageSET_Color3(unsigned int a1, unsigned int op, unsigned int a2, unsigned int a3)
{
	RS.SetColor3(Stage( ), a1, op, a2, a3);
}
void	CBlender_Compile::StageSET_Alpha(unsigned int a1, unsigned int op, unsigned int a2)
{
...
void	CBlender_Compile::Stage_Texture(const char* name, unsigned int, unsigned int fmin, unsigned int fmip, unsigned int fmag)
{
	sh_list& lst = L_textures;
