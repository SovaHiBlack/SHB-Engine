...
inline_ BOOL RayCollider::SegmentAABBOverlap(const Point& center, const Point& extents)
{
	...
	f32 Dx = mData2.x - center.x;		if(_abs(Dx) > extents.x + mFDir.x)	return FALSE;
	f32 Dy = mData2.y - center.y;		if(_abs(Dy) > extents.y + mFDir.y)	return FALSE;
	f32 Dz = mData2.z - center.z;		if(_abs(Dz) > extents.z + mFDir.z)	return FALSE;
...
//	f32 Dx = mOrigin.x - center.x;	if(_abs(Dx) > extents.x && Dx*mDir.x>=0.0f)	return FALSE;
//	f32 Dy = mOrigin.y - center.y;	if(_abs(Dy) > extents.y && Dy*mDir.y>=0.0f)	return FALSE;
//	f32 Dz = mOrigin.z - center.z;	if(_abs(Dz) > extents.z && Dz*mDir.z>=0.0f)	return FALSE;

	f32 Dx = mOrigin.x - center.x;	if(GREATER(Dx, extents.x) && Dx*mDir.x>=0.0f)	return FALSE;
	f32 Dy = mOrigin.y - center.y;	if(GREATER(Dy, extents.y) && Dy*mDir.y>=0.0f)	return FALSE;
	f32 Dz = mOrigin.z - center.z;	if(GREATER(Dz, extents.z) && Dz*mDir.z>=0.0f)	return FALSE;

//	f32 Dx = mOrigin.x - center.x;	if(GREATER(Dx, extents.x) && ((SIR(Dx)-1)^SIR(mDir.x))>=0.0f)	return FALSE;
//	f32 Dy = mOrigin.y - center.y;	if(GREATER(Dy, extents.y) && ((SIR(Dy)-1)^SIR(mDir.y))>=0.0f)	return FALSE;
//	f32 Dz = mOrigin.z - center.z;	if(GREATER(Dz, extents.z) && ((SIR(Dz)-1)^SIR(mDir.z))>=0.0f)	return FALSE;
