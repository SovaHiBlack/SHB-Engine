...
//!!!
		...
		// (almost) Exact sorting order (front-to-back)
			...
			set_Object(0);
			unsigned int uID_LTRACK = 0xffffffff;
	...
	// Determine visibility for static geometry hierrarhy
		for (unsigned int s_it = 0; s_it < PortalTraverser.r_sectors.size( ); s_it++)
		{
			CSector* sector = (CSector*) PortalTraverser.r_sectors[s_it];
			IRender_Visual* root = sector->root( );
			for (unsigned int v_it = 0; v_it < sector->r_frustums.size( ); v_it++)
			{
		...
		// Traverse frustums
		for (unsigned int o_it = 0; o_it < lstRenderables.size( ); o_it++)
		{
			...
			if (PortalTraverser.i_marker != sector->r_marker)	// inactive (untouched) sector
			{
				continue;
			}
...
//.	VERIFY					(g_pGameLevel && g_pGameLevel->pHUD);
	...
	// Update incremental shadowmap-visibility solver
	{
		...
		for (it = 0; it < Lights_LastFrame.size( ); it++)
		{
			...
			{
				Msg("! Failed to flush-OCCq on light [%d] %X", it, *(unsigned int*) (&Lights_LastFrame[it]));
			}
		}
