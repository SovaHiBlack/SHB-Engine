...
void CRenderTarget::u_calc_tc_noise(Fvector2& p0, Fvector2& p1)
{
	...
	unsigned int			tw = iCeil(float(T->get_Width( )) * param_noise_scale + EPS_S);
	unsigned int			th = iCeil(float(T->get_Height( )) * param_noise_scale + EPS_S);
	...
	unsigned int			shift_w = im_noise_shift_w;
	unsigned int			shift_h = im_noise_shift_h;
	float		start_u = (float(shift_w) + 0.5f) / (tw);
	float		start_v = (float(shift_h) + 0.5f) / (th);
	unsigned int			_w = Device.dwWidth;
	unsigned int			_h = Device.dwHeight;
	unsigned int			cnt_w = _w / tw;
	unsigned int			cnt_h = _h / th;
...
void CRenderTarget::u_calc_tc_duality_ss(Fvector2& r0, Fvector2& r1, Fvector2& l0, Fvector2& l1)
{
	...
	float shift_u = param_duality_h * 0.5f;
	float shift_v = param_duality_v * 0.5f;
...
struct TL_2c3uv
{
	...
	unsigned int			color1;
	Fvector2	uv[3];
	inline void	set(float x, float y, unsigned int c0, unsigned int c1, float u0, float v0, float u1, float v1, float u2, float v2)
	{
		p.set(x, y, EPS_S, 1.0f);
		color0 = c0;
		color1 = c1;
		uv[0].set(u0, v0);
		uv[1].set(u1, v1);
...
void CRenderTarget::phase_pp( )
{
	...
	int gblend = clampr(iFloor((1 - param_gray) * 255.0f), 0, 255);
	int nblend = clampr(iFloor((1 - param_noise) * 255.0f), 0, 255);
	unsigned int p_color = subst_alpha(param_color_base, nblend);
	unsigned int p_gray = subst_alpha(param_color_gray, gblend);
	unsigned int p_brightness = param_color_add;
...
//	Msg				("add:  %d,%d,%d",	color_get_R(p_brightness),	color_get_G(p_brightness),	color_get_B(p_brightness));
	...
	// Fill vertex buffer
	float du = ps_r1_pps_u;
	float dv = ps_r1_pps_v;
	...
	RCache.set_c(s_brightness, color_get_R(p_brightness) / 255.0f, color_get_G(p_brightness) / 255.0f, color_get_B(p_brightness) / 255.0f, 0);
