...
private:
	...
	unsigned int											dwAccumulatorClearMark;

public:
	unsigned int											dwLightMarkerID;
	// 
	IBlender*												b_occq;
...
private:
	float													im_noise_time;
	unsigned int											im_noise_shift_w;
	unsigned int											im_noise_shift_h;
	...
	unsigned int											param_color_base;
	unsigned int											param_color_gray;
	unsigned int											param_color_add;
...
public:
	...
	void						u_setrt						(unsigned int W, unsigned int H, IDirect3DSurface9* _1, IDirect3DSurface9* _2, IDirect3DSurface9* _3, IDirect3DSurface9* zb);
	...
	void						phase_smap_direct			(light* L, unsigned int sub_phase);
	void						phase_smap_direct_tsh		(light* L, unsigned int sub_phase);
	...
	void						accum_direct				(unsigned int sub_phase);
	void						accum_direct_f				(unsigned int sub_phase);
	...
	virtual void				set_color_base				(unsigned int f)
	{
		param_color_base = f;
	}
	virtual void				set_color_gray				(unsigned int f)
	{
		param_color_gray = f;
	}
	virtual void				set_color_add				(unsigned int f)
	{
		param_color_add = f;
	}
...
#ifdef DEBUG
	inline void					dbg_addline					(Fvector3& P0, Fvector3& P1, unsigned int c)
	{
	...
	inline void					dbg_addplane				(Fplane& P0, unsigned int c)
	{
