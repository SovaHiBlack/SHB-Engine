...
public:
	...
	inline 	void	seed	(int val)					{ holdrand=val;	}
	inline 	int		maxI	()							{ return 32767;	}

	__declspec (noinline)	int		randI	()							{ return(((holdrand = holdrand * 214013L + 2531011L) >> 16) & 0x7fff); }
	inline 	int		randI	(int max)					{ VERIFY(max);  return randI()%max; }
	inline 	int		randI	(int min, int max)			{ return min+randI(max-min); }
	inline 	int		randIs	(int range)					{ return randI(-range,range); }
	inline 	int		randIs	(int range, int offs)		{ return offs+randIs(range); }
