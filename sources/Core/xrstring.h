...
{ // less
	inline bool		operator ()	(const str_value* A, const str_value* B) const
	{
...
// misc func
	...
	{
		str_value* tmp = p_; p_ = rhs.p_; rhs.p_ = tmp;
	}
...
// res_ptr < res_ptr
// res_ptr > res_ptr
inline bool operator	==	(shared_str const& a, shared_str const& b)
{
...
inline void swap(shared_str& lhs, shared_str& rhs)
{
	lhs.swap(rhs);
}
inline U32	xr_strlen(shared_str& a)
{
	return a.size( );
}
inline int	xr_strcmp(const shared_str& a, const char* b)
{
	return xr_strcmp(*a, b);
}
inline int	xr_strcmp(const char* a, const shared_str& b)
{
	return xr_strcmp(a, *b);
}
