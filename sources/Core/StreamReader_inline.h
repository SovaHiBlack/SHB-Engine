...
inline						CStreamReader::CStreamReader(const CStreamReader& object) : m_start_offset(object.m_start_offset),
...
inline void					CStreamReader::remap(const unsigned int& new_offset)
{
	unmap( );
	map(new_offset);
}

inline unsigned int			CStreamReader::elapsed( ) const
{
	unsigned int offset_from_file_start = tell( );
	VERIFY(m_file_size >= offset_from_file_start);
	return (m_file_size - offset_from_file_start);
}

inline const unsigned int&	CStreamReader::length( ) const
{
	return m_file_size;
}
...
inline unsigned int			CStreamReader::tell( ) const
{
	VERIFY(m_current_pointer >= m_start_pointer);
	VERIFY(unsigned char(m_current_pointer - m_start_pointer) <= m_current_window_size);
