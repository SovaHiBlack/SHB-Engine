...
public:
	...
	IC void		PutCode(int l, u32 c)		/* output c bits of code */
	{
		putbuf |= c >> putlen;
		if ((putlen += l) >= 8)
		{
...
void InsertNode(int r)						/* insert to tree */
{
	...
	for (; ; )
	{
		...
		if (i > THRESHOLD)
		{
			...
			if (i == match_length)
			{
				if ((c = ((r - p) & (N - 1)) - 1) < (u32)match_position)
				{
					match_position = c;
				}
			}
...
void EncodePosition(u32 c)
{
	...
	i = c >> 6;
	fs.PutCode(p_len[i], (u32)p_code[i] << 8);
...
u32 _writeLZ(int hf, pvoid d, u32 size)
{
	u8* start = (u8*)d;
...
void _compressLZ(u8** dest, u32* dest_sz, pvoid src, u32 src_sz)
{
	u8* start = (u8*)src;
...
void _decompressLZ(u8** dest, u32* dest_sz, pvoid src, u32 src_sz)
{
	u8* start = (u8*)src;
	fs.Init_Input(start, start + src_sz);
	Decode();
...
u32 _readLZ(int hf, pvoid& d, u32 size)
{
	// Read file in memory
