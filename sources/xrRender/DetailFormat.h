...
public:
	...
	{
		int _v = iFloor(v * float(range));
		clamp(_v, 0, int(range));
		return _v;
	}
...
//	static void		verify		()						{	VERIFY(16==sizeof(DetailSlot));	}
	void			color_editor( )
	{
		c_dir = w_qclr(0.5f, 15); c_hemi = w_qclr(0.5f, 15); c_r = w_qclr(0.f, 15); c_g = w_qclr(0.f, 15); c_b = w_qclr(0.f, 15);
	}
	unsigned char				r_id(u32 idx)
	{
		switch (idx)
		{
			case 0: return (unsigned char) id0;
			case 1: return (unsigned char) id1;
			case 2: return (unsigned char) id2;
			case 3: return (unsigned char) id3;
			default: NODEFAULT;
		}
#ifdef DEBUG
		return 0;
	...
	void			w_id(u32 idx, unsigned char val)
	{
		switch (idx)
		{
