...
//const f32 W_DIST_FADE		= 15.f;
//const f32	W_DIST_FADE_SQR	= W_DIST_FADE*W_DIST_FADE;
//const f32 I_DIST_FADE_SQR	= 1.f/W_DIST_FADE_SQR;
const s32	MAX_TRIS = 1024;
...
{
	f32 a = 1 - (W->ttl / ps_r__WallmarkTTL);
	s32 aC = iFloor(a * 255.0f);
	clamp(aC, 0, 255);
...
void CWallmarksEngine::RecurseTri(u32 t, fMatrix4x4& mView, CWallmarksEngine::static_wallmark& W)
{
	...
	if (P)
	{
		...
		V0.set((*P)[0], 0, (1 + UV.x) * 0.5f, (1 - UV.y) * 0.5f);
		mView.transform_tiny(UV, (*P)[1]);
		V1.set((*P)[1], 0, (1 + UV.x) * 0.5f, (1 - UV.y) * 0.5f);

		for (u32 i = 2; i < P->size( ); i++)
		{
			mView.transform_tiny(UV, (*P)[i]);
			V2.set((*P)[i], 0, (1 + UV.x) * 0.5f, (1 - UV.y) * 0.5f);
...
void CWallmarksEngine::AddStaticWallmark(CDB::TRI* pTri, const fVector3* pVerts, const fVector3& contact_point, ref_shader hShader, f32 sz)
{
	...
	if (contact_point.distance_to_sqr(Device.vCameraPosition) > _sqr(100.0f))
	{
		return;
	}
...
void CWallmarksEngine::AddSkeletonWallmark(const fMatrix4x4* xf, CKinematics* obj, ref_shader& sh, const fVector3& start, const fVector3& dir, f32 size)
{
	...
	if (xf->c.distance_to_sqr(Device.vCameraPosition) > _sqr(50.0f))
	{
		return;
	}

	VERIFY(obj && xf && (size > EPSILON_3));
	lock.Enter( );
	...
	for (WMSlotVecIt slot_it = marks.begin( ); slot_it != marks.end( ); slot_it++)
	{
		...
		for (StaticWMVecIt w_it = slot->static_items.begin( ); w_it != slot->static_items.end( ); )
		{
			...
			if (RImplementation.ViewBase.testSphere_dirty(W->bounds.P, W->bounds.R))
			{
				...
				f32 dst = Device.vCameraPosition.distance_to_sqr(W->bounds.P);
				f32 ssa = W->bounds.R * W->bounds.R / dst;
				if (ssa >= ssaCLIP)
				{
					u32 w_count = u32(w_verts - w_start);
		...
		for (xr_vector<intrusive_ptr<CSkeletonWallmark> >::iterator w_it = slot->skeleton_items.begin( ); w_it != slot->skeleton_items.end( ); w_it++)
		{
			...
			f32 dst = Device.vCameraPosition.distance_to_sqr(W->m_Bounds.P);
			f32 ssa = W->m_Bounds.R * W->m_Bounds.R / dst;
			if (ssa >= ssaCLIP)
			{
